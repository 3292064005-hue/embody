...
from arm_bt_runtime import BehaviorTreeRuntime, NodeStatus, SequenceNode, TickContext
...
def build_task_behavior_tree() -> BehaviorTreeRuntime:
    """Build the default task-orchestration behavior tree.

    The tree mirrors the current split runtime contract: planner readiness must
    succeed before plan dispatch, and execution dispatch only occurs after a
    successful plan branch.
    """
    return BehaviorTreeRuntime(
        root=SequenceNode(
            children=[
                readiness_condition('motion_planner'),
                status_action('plan_requested', NodeStatus.SUCCESS),
                status_action('execution_requested', NodeStatus.SUCCESS),
            ]
        )
    )
