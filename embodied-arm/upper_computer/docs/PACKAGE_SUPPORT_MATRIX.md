...
| `arm_common`, `arm_interfaces`, `arm_mock_tools`, `arm_sim`, `arm_tools`, `arm_tests` | runtime-support | yes (gate only) | shared support packages may participate in active-lane testing and contract validation, but they are not product-line runtime-core modules; active runtime imports resolve ROS contracts from `arm_interfaces` only |
