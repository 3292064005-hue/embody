...
- `make verify`: 运行 upper_computer fast gate、离线 firmware source contract 校验；若本机已预装对应 PlatformIO platform 包，再执行 ESP32/STM32 实际编译；否则把真实固件编译保留在 CI 或联网工作站执行。
- `make package`: 生成根级 split release manifest 与压缩包。
