...
class TaskActionFacade:
    ...
    def create_servers(self) -> ActionServerSet:
        ...
        if self._HomingAction is not object:
            self._servers.homing = self._ActionServer(
                ...
                goal_callback=lambda _goal: self._GoalResponse.ACCEPT,
    ...
    def pick_place_goal_callback(self, goal_request) -> Any:
        ...
        hardware = getattr(self._node, '_hardware')
        readiness_ok = hardware.is_ready(stale_after_sec=float(self._node.get_parameter('hardware_fresh_sec').value))
        queued = self.build_pick_place_request_from_action(goal_request, task_id=task_id)
        decision = self._orchestrator.accept_request(queued, readiness_ok, 'ready' if readiness_ok else 'hardware not ready')
