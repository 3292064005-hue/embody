...
class CameraDriverNode:
    ...
    def __init__(self, source_type: str = 'mock', *, topic_name: str = DEFAULT_CAMERA_TOPIC, device_index: int = 0, expected_fps: float = 15.0, stale_after_sec: float = DEFAULT_STALE_AFTER_SEC, reconnect_ms: int = 2000) -> None:
        ...
        self._source = MockCameraSource() if source_type == 'mock' else TopicCameraSource()
    ...
    def _source_metadata(self) -> dict[str, Any]:
        ...
        Raises:
            Does not raise.
        """
        return {'sourceType': self.source_type, 'topicName': self.topic_name, 'deviceIndex': self.device_index}
