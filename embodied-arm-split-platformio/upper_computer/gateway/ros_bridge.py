...
from .models import build_command_summary, coerce_system_state_aliases, default_readiness, map_hardware_state_message, map_log_event_message, map_system_state_message, map_target_message, now_iso
...
class RosBridge:
    ...
    async def jog_joint(self, *, joint_index: int, direction: int, step_deg: float) -> None:
        ...
        if self.available and self._node is not None:
            ...
            return
        hardware = self.state.get_hardware()
        joints = list(hardware.get('joints', [0.0] * 5))
