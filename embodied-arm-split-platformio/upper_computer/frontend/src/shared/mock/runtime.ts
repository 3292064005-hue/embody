...
class FixtureReplayRuntime {
  ...
  startTask(payload: Record<string, unknown>): null {
    const taskType = String(payload.taskType || 'pick_place') as TaskProgress['taskType'];
    const targetCategory = payload.targetCategory ? String(payload.targetCategory) : undefined;
    this.currentTask = {
      ...
      updatedAt: new Date().toISOString(),
      targetCategory,
      lastMessage: 'Fixture replay created task placeholder',
    };
    this.system = this.systemSnapshot();
    this.system.runtimePhase = 'perception';
    this.system.taskStage = 'created';
    this.setControllerModeInternal('task');
    this.pushLog({ level: 'info', module: 'mock.fixture', event: 'task.start', taskId: this.currentTask.taskId, message: `Fixture replay: task ${taskType} requested` });
  ...
  stopTask(): null {
    if (this.currentTask) {
      this.currentTask = { ...clone(this.currentTask), stage: 'failed', updatedAt: new Date().toISOString(), lastMessage: 'Fixture replay stopped placeholder task' };
    }
    this.system = this.systemSnapshot();
    this.system.runtimePhase = 'safe_stop';
    this.system.taskStage = 'failed';
