...
def test_sequence_node_resumes_running_child_and_records_trace() -> None:
    ...
    assert context.status_by_node['exec-step'] == 'SUCCESS'
    assert context.status_by_node['tree'] == 'SUCCESS'
    assert context.node_tick_durations_ms['exec-step'] >= 0.0
    assert context.snapshots and context.snapshots[0]['note'] == 'before_tick'
