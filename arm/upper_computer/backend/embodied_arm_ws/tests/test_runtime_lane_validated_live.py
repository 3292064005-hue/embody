...
def _write_validated_live_evidence(
    ...
    *,
    ...
    gate_report: Path | None = None,
    hil_gate_report: Path | None = None,
    checklist_gate_report: Path | None = None,
) -> None:
    gate_report_value = str(gate_report) if gate_report is not None else ''
    hil_gate_report_value = str(hil_gate_report) if hil_gate_report is not None else gate_report_value
    checklist_gate_report_value = str(checklist_gate_report) if checklist_gate_report is not None else gate_report_value
    path.write_text(
        yaml.safe_dump(
            {
                'schema_version': 2,
...
def test_real_validated_live_lane_promotes_from_canonical_effective_receipt(tmp_path: Path, monkeypatch) -> None:
