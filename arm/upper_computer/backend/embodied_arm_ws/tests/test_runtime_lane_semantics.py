...
def test_official_runtime_alias_maps_to_explicit_preview_lane() -> None:
    module = _load_launch_factory_module()
    assert module.normalize_runtime_lane('official_runtime') == 'sim_preview'
    assert module.get_runtime_lane_spec('official_runtime').name == 'sim_preview'
...
def test_lane_specs_preserve_expected_camera_and_transport_modes() -> None:
    ...
    assert module.get_runtime_lane_spec('sim').mock_camera_profile == 'authoritative_demo'
    assert module.get_runtime_lane_spec('sim_perception_realistic').mock_camera_profile == 'realistic_empty'
...
def test_preview_lane_names_are_canonical_while_legacy_names_remain_aliases() -> None:
    module = _load_launch_factory_module()
    assert module.get_runtime_lane_spec('sim_preview').name == 'sim_preview'
    assert module.get_runtime_lane_spec('real_preview').name == 'real_preview'
    assert module.get_runtime_lane_spec('sim').name == 'sim_preview'
    assert module.get_runtime_lane_spec('real').name == 'real_preview'
