...
class SystemStateMachine:
    ...
    def perception_waiting(self, reason: str = "Waiting for authoritative target") -> TransitionResult:
        return self._set(SystemMode.PERCEPTION, reason, FaultCode.NONE, "WAIT_TARGET")

    def perception_blocked(self, reason: str = "No authoritative target available") -> TransitionResult:
        return self._set(SystemMode.PERCEPTION, reason, FaultCode.NONE, "BLOCKED_BY_PERCEPTION")
...
class TaskStateMachine(SystemStateMachine):
    """Blueprint-aligned alias for the event-driven task state machine."""
    pass
