...
from .backend_factory import effective_backend_config_path, list_registered_backend_plugins, resolve_planning_backend
from .moveit_client import MoveItClient
from .planner import MotionPlanner
from .planning_pipeline import list_registered_pipeline_plugins, resolve_postprocessor_plugins, resolve_preprocessor_plugins
...
class MotionPlannerNode(ManagedLifecycleNode):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('planning_preprocessors', [])
        self.declare_parameter('planning_postprocessors', [])
        ...
        self._planning_preprocessor_names = tuple(str(item).strip() for item in (self.get_parameter('planning_preprocessors').value or []) if str(item).strip())
        self._planning_postprocessor_names = tuple(str(item).strip() for item in (self.get_parameter('planning_postprocessors').value or []) if str(item).strip())
        ...
        self._planner = MotionPlanner(
            ...
            preprocessors=resolve_preprocessor_plugins(self._planning_preprocessor_names),
            postprocessors=resolve_postprocessor_plugins(self._planning_postprocessor_names),
        )
    ...
    def _build_scene_provider(self, mode: str):
        ...
        if normalized == 'runtime_service':
            ...
            return build_scene_provider(normalized, client=client, authoritative=True)
        return build_scene_provider(normalized)
    ...
    def _build_grasp_provider(self, mode: str):
        ...
        if normalized == 'runtime_service':
            ...
            return build_grasp_provider(normalized, client=client, authoritative=True)
        return build_grasp_provider(normalized)
    ...
    def _planner_status_payload(self) -> dict[str, Any]:
        return {
            ...
            'planningPreprocessors': list(self._planning_preprocessor_names),
            'planningPostprocessors': list(self._planning_postprocessor_names),
            'planningPipelinePlugins': list_registered_pipeline_plugins(),
            'planningProviderPlugins': list_registered_provider_descriptors(),
            'planningBackendPlugins': list_registered_backend_plugins(),
        }
