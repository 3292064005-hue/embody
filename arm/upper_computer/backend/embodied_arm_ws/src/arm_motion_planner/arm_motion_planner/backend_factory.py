...
@dataclass(frozen=True)
class PlanningBackendPluginDescriptor:
    """Machine-readable description of one planning-backend plugin.

    Attributes:
        name: Stable plugin identifier referenced by backend profiles.
        contract_version: Backend-plugin contract version.
...
PlanningBackendFactory = Callable[[PlanningBackendProfile, str], Callable[[PlanningRequest], PlanResult] | None]


_BACKEND_PLUGIN_REGISTRY: dict[str, tuple[PlanningBackendPluginDescriptor, PlanningBackendFactory]] = {}


def register_backend_plugin(
    name: str,
    factory: PlanningBackendFactory,
    *,
    contract_version: int = 1,
    state: str = 'active',
    description: str = '',
    ...
    """Register one named planning-backend plugin factory.

    Raises:
        ValueError: If the plugin name is empty or already registered and
            ``replace`` is not enabled.
    """
    normalized = str(name or '').strip()
    if not normalized:
        raise ValueError('planning backend plugin name must be non-empty')
    if normalized in _BACKEND_PLUGIN_REGISTRY and not replace:
        raise ValueError(f'planning backend plugin already registered: {normalized}')
    _BACKEND_PLUGIN_REGISTRY[normalized] = (
        PlanningBackendPluginDescriptor(
            name=normalized,
...
def list_registered_backend_plugins() -> list[dict[str, object]]:
    """Return one serializable snapshot of registered planning-backend plugins."""
    payload: list[dict[str, object]] = []
    for key in sorted(_BACKEND_PLUGIN_REGISTRY):
        descriptor = _BACKEND_PLUGIN_REGISTRY[key][0]
        payload.append(
            {
...
def resolve_backend_plugin(name: str) -> PlanningBackendFactory:
    """Resolve one registered planning-backend plugin factory.

    Raises:
        ValueError: If the plugin is not registered.
    """
    normalized = str(name or '').strip()
    record = _BACKEND_PLUGIN_REGISTRY.get(normalized)
    if record is None:
        raise ValueError(f'unsupported planning backend plugin: {name!r}')
    return record[1]
...
register_backend_plugin(
    ...
    lambda profile, backend_name: DeterministicSimulationBackend(profile, backend_name=backend_name),
    description='Deterministic validated-simulation planning backend.',
)
register_backend_plugin(
    'runtime_service_bridge',
    lambda profile, backend_name: RuntimeServicePlanningBackend(profile, backend_name=backend_name) if profile.declared else None,
    description='Runtime-service planning bridge that remains fail-closed until declared.',
)
register_backend_plugin(
    'http_bridge',
    lambda profile, backend_name: HttpPlanningBackend(profile, backend_name=backend_name) if profile.declared else None,
    description='HTTP planning bridge that remains fail-closed until declared.',
)
...
def resolve_planning_backend(
    *,
    ...
    factory = resolve_backend_plugin(profile.plugin)
    backend_callable = factory(profile, backend_name)
    return ResolvedPlanningBackend(
