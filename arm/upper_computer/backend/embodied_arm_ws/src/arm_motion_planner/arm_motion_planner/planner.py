...
from .planning_pipeline import PlanningPipeline, PlanningPipelineState, PlanningPostprocessor, PlanningPreprocessor
...
class MotionPlanner:
    ...
    def __init__(
        ...
        *,
        ...
        preprocessors: tuple[PlanningPreprocessor, ...] | None = None,
        postprocessors: tuple[PlanningPostprocessor, ...] | None = None,
    ) -> None:
        ...
        self._preprocessors = tuple(preprocessors or ())
        self._postprocessors = tuple(postprocessors or ())
        self._pipeline = PlanningPipeline(
            preprocessors=(self._default_validation_preprocessor, *self._preprocessors),
    ...
    def _default_validation_preprocessor(self, state: PlanningPipelineState) -> PlanningPipelineState:
        self._validate_target(state.target)
        state.place_pose = self._validate_place_pose(state.place_pose)
        if not (state.calibration.pre_grasp_z > state.calibration.grasp_z and state.calibration.retreat_z >= state.calibration.pre_grasp_z):
            raise InvalidTargetError('invalid calibration z ordering')
        return state

    def _run_preprocessors(self, state: PlanningPipelineState) -> PlanningPipelineState:
        return self._pipeline.run_preprocessors(state)

    def _run_postprocessors(self, state: PlanningPipelineState, plan: list[StagePlan]) -> list[StagePlan]:
        return self._pipeline.run_postprocessors(state, plan)
    ...
    def build_pick_place_plan(self, context: TaskContext, target: TargetSnapshot, calibration: CalibrationProfile) -> list[StagePlan]:
        ...
        state = PlanningPipelineState(
            context=context,
            target=self._normalize_target_snapshot(target),
            calibration=calibration,
            place_pose=dict(context.active_place_pose or calibration.resolve_place_profile(context.place_profile)),
        )
        state.metadata.setdefault('pipelineProfile', 'pick_place_v1')
        state = self._run_preprocessors(state)
        grasp_plan = self._grasp_planner.plan(state.target.to_dict(), {**state.place_pose, 'z': state.calibration.place_z})
        candidate = dict(grasp_plan.get('candidate') or {})
        grasp_x = float(candidate.get('grasp_x', state.target.table_x))
        grasp_y = float(candidate.get('grasp_y', state.target.table_y))
        grasp_yaw = float(candidate.get('yaw', state.target.yaw))
        grasp_x, grasp_y, grasp_yaw = state.calibration.apply_target(grasp_x, grasp_y, grasp_yaw)
        scene_snapshot = self._scene_manager.sync_scene({'target': state.target.to_dict()})
        common = {
            'frame': 'table',
            'taskId': context.task_id,
            'targetId': state.target.target_id,
            ...
            'planningPipelineMetadata': dict(state.metadata),
        }
        plan = [
            StagePlan('move_to_pregrasp', 'connector', {'x': grasp_x, 'y': grasp_y, 'z': state.calibration.pre_grasp_z, 'yaw': grasp_yaw, 'timeoutSec': 1.0, **common}),
            StagePlan('descend', 'propagator', {'x': grasp_x, 'y': grasp_y, 'z': state.calibration.grasp_z, 'yaw': grasp_yaw, 'timeoutSec': 0.8, **common}),
            StagePlan('close_gripper', 'gripper', {'open': False, 'timeoutSec': 0.6, **common}),
            StagePlan('lift', 'propagator', {'x': grasp_x, 'y': grasp_y, 'z': state.calibration.retreat_z, 'yaw': grasp_yaw, 'timeoutSec': 0.8, **common}),
            StagePlan('move_to_place', 'connector', {**state.place_pose, 'z': state.calibration.place_z, 'timeoutSec': 1.0, **common}),
            StagePlan('open_gripper', 'gripper', {'open': True, 'timeoutSec': 0.6, **common}),
            StagePlan('retreat', 'propagator', {'x': state.place_pose['x'], 'y': state.place_pose['y'], 'z': state.calibration.retreat_z, 'yaw': state.place_pose['yaw'], 'timeoutSec': 0.8, **common}),
            StagePlan('go_home', 'connector', {'named_pose': 'home', 'timeoutSec': 1.0, **common}),
        ]
        return self._run_postprocessors(state, plan)
