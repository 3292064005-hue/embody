...
EXECUTION_PRODUCER_REGISTRY_PATH = Path(__file__).resolve().parents[2] / 'arm_bringup' / 'config' / 'runtime_authority_sections' / 'capability_registry.yaml'
...
class HardwareCommandDispatcherNode(ManagedLifecycleNode):
    ...
    @staticmethod
    def _load_execution_producer_contracts() -> dict[str, dict[str, Any]]:
        """Load declared execution-producer contracts from the authority registry.

        Returns:
            dict[str, dict[str, Any]]: Mapping keyed by producer name.

        Raises:
            Does not raise. Missing or unreadable registry files degrade to an
            empty mapping so the dispatcher can fall back to the conservative
            built-in allowlist.
        """
        if yaml is None or not EXECUTION_PRODUCER_REGISTRY_PATH.exists():
            return {}
        try:
            payload = yaml.safe_load(EXECUTION_PRODUCER_REGISTRY_PATH.read_text(encoding='utf-8')) or {}
        except Exception:
            return {}
        capability_registry = payload.get('capability_registry', {}) if isinstance(payload, dict) else {}
        producers = capability_registry.get('execution_producers', {}) if isinstance(capability_registry, dict) else {}
        if not isinstance(producers, dict):
            return {}
        return {str(name): dict(contract) for name, contract in producers.items() if isinstance(contract, dict)}

    @classmethod
    def _execution_producer_contracts(cls) -> dict[str, dict[str, Any]]:
        contracts = getattr(cls, '_cached_execution_producer_contracts', None)
        if isinstance(contracts, dict):
            return contracts
        contracts = cls._load_execution_producer_contracts()
        cls._cached_execution_producer_contracts = contracts
        return contracts

    @classmethod
    def _producer_contract(cls, producer: str) -> dict[str, Any]:
        if not producer:
            return {}
        return dict(cls._execution_producer_contracts().get(str(producer), {}))

    @staticmethod
    def _validate_command_origin(payload: Dict[str, Any]) -> None:
        """Validate origin metadata for execution-bound hardware commands.
        ...
        Raises:
            SafetyViolation: Raised when a joint-stream producer is undeclared,
                inactive, or bound to the wrong command plane.

        Boundary behavior:
            When the authority registry is unavailable, validation degrades to the
            conservative built-in allowlist rather than silently allowing new
            producers.
        """
        ...
        contract = HardwareCommandDispatcherNode._producer_contract(producer)
        authorized = False
        if contract:
            authorized = (
                str(contract.get('state', '') or '').strip() == 'active'
                and str(contract.get('capability', '') or '').strip() == 'joint_stream'
            )
        elif producer in JOINT_STREAM_PRODUCERS:
            authorized = True
        if not authorized:
        ...
        expected_command_plane = str(contract.get('command_plane', '') or 'joint_stream').strip() if contract else 'joint_stream'
        if command_plane and command_plane != expected_command_plane:
            raise SafetyViolation(
                'hardware_dispatcher SET_JOINTS: command_plane must be '
                f'{expected_command_plane} when provided'
            )
