...
def _build_trace_envelope(*, request_id: str = '', task_id: str = '', correlation_id: str = '', task_run_id: str = '', episode_id: str = '') -> dict[str, str]:
    """Build one canonical split-stack trace envelope.

    Args:
        request_id: Stable request identifier for the current message.
        task_id: Task identifier associated with the current runtime flow.
        correlation_id: Cross-surface distributed trace identifier.
        task_run_id: Stable task-run identifier.
        episode_id: Stable episode identifier. When omitted it degrades to
            ``task_run_id`` so replay/audit clients see a single truth source.

    Returns:
        dict[str, str]: Canonical trace envelope used by planning/execution
            request/result/status payloads.

    Raises:
        Does not raise. Missing fields degrade to empty strings.
    """
    return {
...
def build_planning_request(*, request_id: str, context: TaskContext, target: TargetSnapshot, calibration: CalibrationProfile, correlation_id: str = '', task_run_id: str = '', episode_id: str = '') -> dict[str, Any]:
    ...
    return {
        ...
        'traceEnvelope': _build_trace_envelope(request_id=request_id, task_id=context.task_id, correlation_id=correlation_id, task_run_id=task_run_id, episode_id=episode_id),
        'context': {
...
def build_planning_result(*, request_id: str, task_id: str, accepted: bool, message: str, plan: list[StagePlan] | None = None, correlation_id: str = '', task_run_id: str = '', episode_id: str = '') -> dict[str, Any]:
    ...
    return {
        ...
        'episodeId': str(episode_id or task_run_id),
        'taskId': str(task_id),
        'traceEnvelope': _build_trace_envelope(request_id=request_id, task_id=task_id, correlation_id=correlation_id, task_run_id=task_run_id, episode_id=episode_id),
