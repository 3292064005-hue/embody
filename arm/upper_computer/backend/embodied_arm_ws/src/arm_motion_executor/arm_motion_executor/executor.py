...
class MotionExecutor:
    ...
    def build_command_stream(self, plan: Iterable[StagePlan], task_id: str, *, request_metadata: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        """Build serialized controller commands for a stage plan.

        Args:
            plan: Stage plan iterable.
            task_id: Task identifier.
            request_metadata: Optional runtime request correlation fields copied
                onto each command payload before transport dispatch.
        ...
        request_id = str(request_metadata.get('request_id', request_metadata.get('requestId', '')))
        correlation_id = str(request_metadata.get('correlation_id', request_metadata.get('correlationId', '')))
        task_run_id = str(request_metadata.get('task_run_id', request_metadata.get('taskRunId', '')))
        stage_count = 0
    ...
    def accept_feedback(self, feedback: dict[str, Any]) -> ExecutionResult:
        ...
        return ExecutionResult(status not in {'failed', 'timeout', 'fault', 'canceled', 'nack'}, len(self._state.handles), handle.message or status, status=status, stage_name=handle.stage_name, command_id=command_id, feedback_source=handle.feedback_source)

    def mark_failed(self, command_id: str, *, message: str, source: str = 'executor', status: str = 'failed') -> ExecutionResult:
        """Mark one tracked execution handle as failed.

        Args:
            command_id: Failed command identifier.
            message: Human-readable failure detail.
            source: Reporting component.
            status: Terminal executor status to store. Use ``failed`` for
                business-level failures and ``fault`` for raw hardware fault
                transitions that must still stop the execution handle.

        Returns:
            ExecutionResult: Failure outcome.
        ...
        handle = self._state.handles.get(command_id)
        if handle is None:
            return ExecutionResult(False, len(self._state.handles), 'unknown command failure', status=status, command_id=command_id, feedback_source=source)
        handle.result_status = str(status)
        handle.feedback_source = source
        handle.message = str(message)
        self._state.status = str(status)
        return ExecutionResult(False, len(self._state.handles), handle.message, status=str(status), stage_name=handle.stage_name, command_id=command_id, feedback_source=source)

    def mark_all_failed(self, *, message: str, source: str = 'executor', status: str = 'failed') -> list[ExecutionResult]:
        """Mark every non-terminal execution handle as failed.

        Args:
            message: Human-readable failure detail applied to each handle.
            source: Reporting component.
            status: Terminal state persisted to each handle.

        Returns:
            list[ExecutionResult]: Failure results for every updated handle.

        Raises:
            Does not raise.
        """
        terminal_statuses = {'done', 'failed', 'timeout', 'canceled', 'fault'}
        results: list[ExecutionResult] = []
        for command_id, handle in self._state.handles.items():
            if handle.result_status in terminal_statuses:
                continue
            results.append(self.mark_failed(command_id, message=message, source=source, status=status))
        return results
