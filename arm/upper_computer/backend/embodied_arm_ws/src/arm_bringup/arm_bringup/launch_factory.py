...
def _load_gate_report(path_value: str) -> dict[str, Any]:
    path_text = str(path_value or '').strip()
    if not path_text:
        return _load_target_runtime_gate_report()
    path = Path(path_text).expanduser()
    if not path.is_absolute():
        path = UPPER_COMPUTER_ROOT / path
    if not path.exists():
        return {}
    try:
        import json
...
def _artifact_status_tokens(marker: str) -> tuple[str, ...]:
    if marker == 'target_runtime_gate_passed':
        return ('current status: `passed`', 'current status: passed')
    return ('status: **passed**', 'status: `passed`', 'status: passed')


def _validated_live_artifact_text_consistent(marker: str, artifact_path: Path) -> bool:
    text = artifact_path.read_text(encoding='utf-8', errors='replace').lower()
    if not any(token in text for token in _artifact_status_tokens(marker)):
        return False
    return not any(token in text for token in ('not_passed', 'not_signed', 'not_executed', 'blocked', 'failed'))
...
def _validated_live_artifact_passed(marker: str) -> bool:
    ...
    if not artifact:
        return False
    artifact_path = UPPER_COMPUTER_ROOT / artifact
    if not artifact_path.exists() or not _validated_live_artifact_text_consistent(marker, artifact_path):
        return False
    gate_field = str(item.get('gate_field', '') or '').strip()
    if gate_field:
        report = _load_gate_report(str(item.get('gate_report', '') or ''))
...
def _validated_live_target_gate_passed() -> bool:
    ...
    # The validated-live target-runtime marker tracks the target gate itself.
    # Repository-lane or final release blockers must not retroactively downgrade
    # a passed target gate into a preview-only live lane.
    return str(report.get('targetGate', '') or '').strip().lower() == 'passed'
...
def _receipt_effective(payload: dict[str, Any] | None, *, spec: RuntimeLaneSpec, planning_backend_declared: bool) -> bool:
    ...
    declared_evidence = {str(item).strip() for item in data.get('evidence', []) if str(item).strip()}
    if any(marker not in declared_evidence for marker in required_evidence):
        return False
