...
import { fetchTargets, fetchVisionFrame, fetchCalibrationProfile, fetchCalibrationVersions } from '@/services/api/vision';
...
import { fetchAuditLogs, fetchCommandReceipts, fetchLogs } from '@/services/api/logs';
...
export function useServerStateSync(): void {
  ...
  const resources: ResourceDefinition[] = [
    ...
    { topic: 'vision.frame', intervalMs: 2000, fetcher: fetchVisionFrame, apply: (payload) => visionStore.setFrame(payload as Awaited<ReturnType<typeof fetchVisionFrame>>), mode: 'poll' },
    ...
    { topic: 'receipts', intervalMs: 5000, fetcher: fetchCommandReceipts, apply: (payload) => receiptStore.setRecords(payload as Awaited<ReturnType<typeof fetchCommandReceipts>>), mode: 'poll' }
  ];
