...
async function resolveMock(config: AxiosRequestConfig): Promise<AxiosResponse> {
  ...
  if (method === 'post' && url === '/api/system/recover') return buildResponse(config, mockRuntime.recoverRuntime());
  ...
  if (method === 'get' && url === '/api/vision/frame') return buildResponse(config, mockRuntime.getVisionFrame());
  ...
  if ((method === 'put' || method === 'post') && /\/api\/calibration\/(profiles\/[^/]+\/activate|activate)$/.test(url)) {
    ...
    const profileIdFromUrl = urlSegments.length >= 2 ? urlSegments[urlSegments.length - 2] : '';
  ...
  if (method === 'get' && url === '/api/logs/receipts') return buildResponse(config, mockRuntime.getCommandReceipts());
