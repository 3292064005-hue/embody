<template>
  <header class="top-bar panel">
    ...
    <div class="state-group">
      ...
      <div :class="['status-pill', authorityPillClass]">
        <span class="status-dot"></span>
        <span>AUTH {{ authorityPillLabel }}</span>
      </div>
      <div :class="['status-pill', taskExecutionPillClass]">
        <span class="status-dot"></span>
        <span>TASK {{ taskExecutionPillLabel }}</span>
      </div>
      <div :class="['status-pill', commandPillClass]">
        <span class="status-dot"></span>
        <span>CMD {{ readinessStore.commandSummary?.readyCount || 0 }}/{{ totalCommandCount }}</span>
      </div>
      <div :class="['status-pill', lifecyclePillClass]">
        <span class="status-dot"></span>
        <span>LIFECYCLE {{ commandLifecycleStore.pendingCount }}</span>
      </div>
      ...
      <div :class="['status-pill', systemStore.current?.esp32Connected ? 'status-ok' : 'status-danger']">
        <span class="status-dot"></span>
        <span>ESP32</span>
      </div>
      <div :class="['status-pill', cameraIngressClass]">
        <span class="status-dot"></span>
        <span>{{ cameraIngressLabel }}</span>
      </div>
      <el-tag>CTRL {{ controllerModeLabel }}</el-tag>
      <el-tag>TASK {{ taskStage }}</el-tag>
      <el-tag>BUILD {{ buildProfileLabel }}</el-tag>
      <el-tag v-if="readinessStore.runtimeFingerprint">FP {{ readinessStore.runtimeFingerprint }}</el-tag>
      <el-tag v-if="readinessStore.simulated" type="warning">SIM_RUNTIME</el-tag>
      <el-tag v-if="hardwarePreviewExecution" type="warning">PREVIEW_EXEC</el-tag>
...
const lifecyclePillClass = computed(() => commandLifecycleStore.pendingCount ? 'status-warn' : 'status-ok');
const hardwareSimulated = computed(() => Boolean(robotStore.hardware?.sourceStm32Simulated));
const hardwarePreviewExecution = computed(() => Boolean(robotStore.hardware?.sourceStm32ExecutionPreviewOnly));
...
const cameraIngressClass = computed(() => (robotStore.hardware?.sourceCameraFrameIngressLive || systemStore.current?.cameraConnected) ? 'status-ok' : (robotStore.hardware?.sourceEsp32StreamReserved ? 'status-warn' : 'status-danger'));
const cameraIngressLabel = computed(() => (robotStore.hardware?.sourceCameraFrameIngressLive || systemStore.current?.cameraConnected) ? 'FRAME' : (robotStore.hardware?.sourceEsp32StreamReserved ? 'STREAM RESERVED' : 'FRAME OFF'));
const authorityPillLabel = computed(() => String(readinessStore.authorityState?.level || 'bootstrap').replace(/_/g, ' ').toUpperCase());
const authorityPillClass = computed(() => {
  const level = readinessStore.authorityState?.level || 'bootstrap';
  if (level === 'authoritative_transport' || level === 'authoritative_runtime') return 'status-ok';
  if (level === 'local_preview' || level === 'bootstrap') return 'status-warn';
  return 'status-danger';
});
const taskExecutionPillLabel = computed(() => readinessStore.taskExecutionState?.interactive ? 'INTERACTIVE' : readinessStore.taskExecutionState?.workbenchVisible ? 'VISIBLE' : 'BLOCKED');
const taskExecutionPillClass = computed(() => readinessStore.taskExecutionState?.interactive ? 'status-ok' : readinessStore.taskExecutionState?.workbenchVisible ? 'status-warn' : 'status-danger');
const buildProfileLabel = computed(() => String(import.meta.env.VITE_BUILD_PROFILE || 'development').toUpperCase());
...
async function emitEmergencyStop() {
  try {
    ...
    ElNotification.warning({ title: systemStore.localPreviewOnly ? '急停仅做本地 preview 投影' : '急停命令已受理', message: systemStore.localPreviewOnly ? (systemStore.localPreviewMessage || '当前急停命令仅做本地 preview 投影，未下发到权威运行时。') : '请以后端网关和硬件状态回执为准。' });
  } catch {
    // canceled or blocked
  }
}
