...
export function deriveHardwareTransportLabel(hardware: HardwareState | null | undefined): string {
  ...
  if (hardware?.sourceStm32ExecutionSemantics === 'authoritative_simulated_transport') return 'STM32 AUTH-SIM';
  if (hardware?.sourceStm32ExecutionSemantics === 'preview_simulated_transport') return 'STM32 PREVIEW';
  if (hardware?.sourceStm32SimulatedFallback) return 'STM32 FALLBACK';
  if (hardware?.sourceStm32Simulated) return 'STM32 SIM';
  if (hardware?.sourceStm32ExecutionSemantics === 'non_authoritative_real_link') return 'STM32 LINK ONLY';
...
export function deriveHardwareTransportClass(hardware: HardwareState | null | undefined): string {
  ...
  if (hardware?.sourceStm32ExecutionSemantics === 'authoritative_simulated_transport') return 'status-ok';
  if (hardware?.sourceStm32ExecutionPreviewOnly || hardware?.sourceStm32SimulatedFallback || hardware?.sourceStm32Simulated) return 'status-warn';
