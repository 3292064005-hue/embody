  ...
  <section class="panel task-control">
    ...
    <div class="controls">
      <template v-if="showTaskWorkbench">
        ...
        <div v-if="startMissingDetails.length" class="subtle missing-summary">
          当前缺失：{{ startMissingDetails.join(' / ') }}
        </div>
...
async function handleStart() {
  try {
    ...
    ElMessage.success(`任务启动命令已受理：${template.name}`);
  } catch (error) {
...
async function handleStop() {
  try {
    ...
    ElMessage.success(taskStore.localPreviewOnly ? '停止任务命令仅做本地 preview 投影' : '停止任务命令已受理');
  } catch (error) {
    if (error instanceof Error) ElMessage.warning(error.message);
  }
}
...
.missing-summary { line-height: 1.7; }
</style>
