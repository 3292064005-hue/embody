...
export const useSystemStore = defineStore('system', {
  ...
  actions: {
    setState(payload: SystemState) {
      const normalized = coerceSystemState(payload as Partial<SystemState> & Record<string, unknown>);
      this.current = normalized;
      useCommandLifecycleStore().reconcileSystem(normalized);
    },
    applyCommandTransportResult(result?: { localPreviewOnly?: boolean; message?: string }) {
      ...
      this.localPreviewMessage = this.localPreviewOnly ? String(result?.message || '当前系统命令仅做本地 preview 投影；若需确认完成态，请以后端系统状态与命令回执为准。') : '';
    },
    async homeRobot() {
      ...
      try { const result = await homeRobot(); this.applyCommandTransportResult(result); lifecycleStore.registerAccepted('system.home', result, { message: '回零命令已受理，等待权威系统/硬件状态更新', expectation: { runtimePhase: 'idle' } }); auditStore.updateStatus(audit.id, 'accepted', this.localPreviewOnly ? '回零命令仅进入本地 preview 投影' : '回零命令已受理，等待系统状态更新'); invalidateResources(['system', 'hardware', 'logs']); }
    ...
    async resetFault() {
      ...
      try { const result = await resetFault(); this.applyCommandTransportResult(result); lifecycleStore.registerAccepted('system.reset_fault', result, { message: '故障复位命令已受理，等待权威系统状态更新', expectation: { runtimePhase: 'idle' } }); auditStore.updateStatus(audit.id, 'accepted', this.localPreviewOnly ? '故障复位命令仅进入本地 preview 投影' : '故障复位命令已受理，等待系统状态更新'); invalidateResources(['system', 'hardware', 'logs', 'task.current']); }
    ...
    async recoverRuntime() {
      ...
      try { const result = await recoverRuntime(); this.applyCommandTransportResult(result); lifecycleStore.registerAccepted('system.recover', result, { message: '运行时恢复命令已受理，等待权威系统状态更新', expectation: { runtimePhase: 'idle' } }); auditStore.updateStatus(audit.id, 'accepted', this.localPreviewOnly ? '运行时恢复命令仅进入本地 preview 投影' : '运行时恢复命令已受理，等待系统状态更新'); invalidateResources(['system', 'hardware', 'logs', 'task.current', 'readiness']); }
      catch (error) { auditStore.updateStatus(audit.id, 'failed', error instanceof Error ? error.message : '运行时恢复失败'); throw error; }
      finally { this.pendingCommand = false; }
    },
    async emergencyStop() {
      if (this.pendingCommand) return;
      const appStore = useAppStore(); const safetyStore = useSafetyStore(); const auditStore = useAuditStore(); const lifecycleStore = useCommandLifecycleStore();
      this.pendingCommand = true;
      ...
      try { const result = await emergencyStop(); this.applyCommandTransportResult(result); lifecycleStore.registerAccepted('system.emergency_stop', result, { message: '急停命令已受理，等待权威系统状态更新', expectation: { runtimePhase: 'safe_stop', emergencyStop: true } }); auditStore.updateStatus(audit.id, 'accepted', this.localPreviewOnly ? '急停命令仅进入本地 preview 投影' : '急停命令已受理，等待系统状态更新'); invalidateResources(['system', 'hardware', 'logs', 'task.current']); }
