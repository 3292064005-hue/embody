...
export const useAuditStore = defineStore('audit', {
  ...
  actions: {
    setRecords(records: Array<Partial<CommandAuditRecord> | (Partial<CommandAuditRecord> & { [key: string]: unknown })>) {
      const normalized = records.map((record) => normalizeRecord(record as Partial<CommandAuditRecord> & { [key: string]: unknown })).slice(0, 500);
      this.records = normalized;
      const lifecycleStore = useCommandLifecycleStore();
      normalized.forEach((item) => lifecycleStore.mergeAudit(item));
    },
    mergeServerRecord(record: Partial<CommandAuditRecord> | (Partial<CommandAuditRecord> & { [key: string]: unknown })) {
      ...
      useCommandLifecycleStore().mergeAudit(normalized);
    },
