...
export const useTaskStore = defineStore('task', {
  ...
  actions: {
    setCurrentTask(payload: TaskProgress | null) {
      this.currentTask = payload;
      useCommandLifecycleStore().reconcileTask(payload);
    },
    applyCommandTransportResult(result?: { localPreviewOnly?: boolean; message?: string }) {
      ...
      this.localPreviewMessage = this.localPreviewOnly ? String(result?.message || '当前任务命令仅做本地 preview 投影；若需确认完成态，请以后端任务状态与命令回执为准。') : '';
    },
    ...
    async startTask(payload: StartTaskPayload) {
      ...
      this.pending = true;
      const audit = auditStore.createEntry({ action: 'task.start', actorRole: appStore.operatorRole, reason: '命令已受理，等待权威运行时回执', payload: payload as unknown as Record<string, unknown>, guardSummary: safetyStore.guardSummary });
      try {
        const decision = await startTask(payload);
        lifecycleStore.registerAccepted('task.start', decision, {
          message: `启动任务命令已受理（taskId=${decision.taskId}，run=${decision.taskRunId}）`,
          payload: payload as unknown as Record<string, unknown>,
          expectation: { taskId: decision.taskId, taskRunId: decision.taskRunId },
        });
        auditStore.updateStatus(audit.id, 'accepted', `启动任务命令已受理（taskId=${decision.taskId}，run=${decision.taskRunId}，op=${decision.operationId || '--'}）`);
    ...
    async stopTask() {
      ...
      try {
        ...
        lifecycleStore.registerAccepted('task.stop', result, { message: '停止任务命令已受理，等待权威任务状态更新' });
        auditStore.updateStatus(audit.id, 'accepted', this.localPreviewOnly ? '停止任务命令仅进入本地 preview 投影' : '停止任务命令已受理，等待任务/系统状态更新');
        invalidateResources(['task.current', 'task.history', 'system', 'logs', 'hardware']);
      } catch (error) {
