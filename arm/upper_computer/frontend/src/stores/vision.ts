...
const emptyFrame: VisionFrame = { available: false, targetCount: 0, message: 'frame stream unavailable' };

export const useVisionStore = defineStore('vision', {
  state: (): { targets: VisionTarget[]; frame: VisionFrame; calibration: CalibrationProfile | null; versions: CalibrationProfileVersion[]; loading: boolean; saving: boolean; selectedVersionId: string } => ({
    targets: [], frame: { ...emptyFrame }, calibration: null, versions: [], loading: false, saving: false, selectedVersionId: ''
  }),
  getters: {
  ...
  actions: {
    ...
    setFrame(payload: VisionFrame | null) { this.frame = payload ? { ...emptyFrame, ...payload } : { ...emptyFrame }; },
