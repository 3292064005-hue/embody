...
const emptyReadiness: ReadinessState = {
  ...
  executionBackboneSummary: {
    ...
    effectiveBackboneLabel: 'Inactive Backbone',
    effectiveExecutionModeLabel: 'inactive',
  ...
  runtimeFeatureState: undefined,
  authorityState: undefined,
  commandSurfaceState: undefined,
  taskExecutionState: undefined,
  runtimeFingerprint: '',
  runtimeSurfaceState: undefined,
  runtimeTier: 'preview',
  productLine: 'Preview / Contract Only',
...
export const useReadinessStore = defineStore('readiness', {
  ...
  actions: {
    setReadiness(payload: ReadinessState) {
      ...
      this.commandPlanes = payload.commandPlanes || {};
      this.capabilityDescriptors = payload.capabilityDescriptors || {};
      this.runtimeFeatureState = payload.runtimeFeatureState || this.runtimeFeatureState;
      this.authorityState = payload.authorityState || payload.runtimeSurfaceState?.authorityState || this.authorityState;
      this.commandSurfaceState = payload.commandSurfaceState || payload.runtimeSurfaceState?.commandSurfaceState || this.commandSurfaceState;
      this.taskExecutionState = payload.taskExecutionState || payload.runtimeSurfaceState?.taskExecutionState || this.taskExecutionState;
      this.runtimeFingerprint = payload.runtimeFingerprint || payload.runtimeSurfaceState?.runtimeFingerprint || '';
      this.runtimeSurfaceState = payload.runtimeSurfaceState || this.runtimeSurfaceState;
      this.runtimeTier = payload.taskExecutionState?.runtimeTier || payload.runtimeSurfaceState?.runtimeTier || payload.runtimeTier || this.runtimeTier || 'preview';
      this.productLine = payload.runtimeSurfaceState?.runtimeLabel || payload.productLine || this.productLine || 'Preview / Contract Only';
      this.runtimeDeliveryTrack = payload.runtimeSurfaceState?.runtimeDeliveryTrack || payload.runtimeDeliveryTrack || this.runtimeDeliveryTrack;
      this.executionBackbone = payload.runtimeSurfaceState?.executionBackbone || payload.executionBackbone || this.executionBackbone;
