<template>
  <section class="task-page">
    <div class="grid-4 summary-grid">
      ...
      <MetricCard label="执行背板" :value="executionBackboneLabel" :hint="executionModeLabel" :tone="readinessStore.executionBackboneSummary?.effectiveTransportReady ? 'success' : 'warning'" />
    ...
    <div class="grid-2 content-grid">
      <div class="panel page-panel">
        ...
        <div class="meta-grid" style="margin-top: 12px">
          ...
          <div><strong>authority：</strong>{{ readinessStore.authorityState?.level || '--' }}</div>
          <div><strong>task execution：</strong>{{ readinessStore.taskExecutionState?.interactive ? 'interactive' : readinessStore.taskExecutionState?.workbenchVisible ? 'visible' : 'blocked' }}</div>
          <div class="gate-wide"><strong>runtime fingerprint：</strong>{{ readinessStore.runtimeFingerprint || '--' }}</div>
        </div>
        <div class="subtle" style="margin-top: 10px">{{ readinessStore.authorityState?.detail || readinessStore.firmwareSemanticMessage || '固件语义未上报。' }}</div>
      </div>
      <div class="panel page-panel">
    ...
    <div v-if="runtimeFeatures.taskWorkbenchVisible" class="panel page-panel">
      ...
      <div v-if="startMissingDetails.length" class="subtle page-note">缺失项：{{ startMissingDetails.join(' / ' ) }}</div>
    ...
    <div class="panel page-panel">
      ...
      <div v-if="runtimeFeatures.previewRuntime" class="subtle page-note">当前只开放历史与审计信息；任务模板和发令工作台仅在 authoritative runtime 中暴露。</div>
      <div v-else-if="!runtimeFeatures.taskExecutionInteractive" class="subtle page-note">任务工作台已开放；当前仅禁止 startTask，请先补齐缺失项后再发令。</div>
...
const executionBackboneLabel = computed(() => {
  const summary = readinessStore.executionBackboneSummary;
  return summary?.effectiveBackboneLabel || summary?.backboneLabel || 'Unknown Backbone';
});
const executionModeLabel = computed(() => {
  const summary = readinessStore.executionBackboneSummary;
  const effective = summary?.effectiveExecutionModeLabel || summary?.effectiveExecutionMode || '';
  const declared = summary?.executionModeLabel || summary?.executionMode || '';
  if (effective && effective !== declared && declared) {
    return `${effective}（declared: ${declared}）`;
  }
  return effective || declared || '--';
});
const startMissingDetails = computed(() => (readinessStore.taskExecutionState?.startMissingDetails || []).map((item) => `${item.name}(${item.detail})`));
