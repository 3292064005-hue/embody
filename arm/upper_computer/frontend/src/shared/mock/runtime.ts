...
function createReadiness(system: SystemState): ReadinessState {
  return {
    ...
    authorityState: {
      ...
      localPreview: true,
      bootstrap: false,
      maintenanceCommandReady: true,
      detail: 'Fixture replay is a non-authoritative local preview surface.',
    },
    commandSurfaceState: {
      surface: 'local_preview',
      interactive: true,
      allowedCommands: ['stopTask', 'home', 'recover'],
      blockedCommands: ['startTask'],
      readyCount: 3,
      blockedCount: 1,
      startTaskAllowed: false,
      detail: 'Fixture replay projects gateway-visible command surfaces without authoritative transport.',
    },
    taskExecutionState: {
      startAllowed: false,
      ...
      promotionMissing: ['authoritative_runtime'],
      runtimeTier: 'preview',
      reason: 'Fixture replay keeps task execution hidden outside authoritative validated lanes.',
    },
    runtimeTier: 'preview',
    productLine: 'preview',
    runtimeFingerprint: 'preview:frontend_fixture_replay',
    manualCommandLimits: {
...
class FixtureReplayRuntime {
  ...
  homeRobot() {
    ...
    this.pushAudit('system.home', 'accepted', 'Fixture replay: home command accepted');
    return { accepted: true, localPreviewOnly: false, commandMode: 'fixture_mock', message: 'Fixture replay: home acknowledged', mode: this.system.controllerMode || 'idle' };
  }
  ...
  resetFault() {
    ...
    this.pushAudit('system.reset_fault', 'accepted', 'Fixture replay: reset-fault command accepted');
    return { accepted: true, localPreviewOnly: false, commandMode: 'fixture_mock', message: 'Fixture replay: reset fault acknowledged', mode: this.system.controllerMode || 'idle' };
  }
  ...
  emergencyStop() {
    ...
    this.pushAudit('system.emergency_stop', 'accepted', 'Fixture replay: emergency-stop command accepted');
    return { accepted: true, localPreviewOnly: false, commandMode: 'fixture_mock', message: 'Fixture replay: emergency stop asserted', mode: this.system.controllerMode || 'maintenance' };
  }
  ...
  recoverRuntime() {
    ...
    this.pushAudit('system.recover', 'accepted', 'Fixture replay: runtime recover command accepted');
    return { accepted: true, localPreviewOnly: false, commandMode: 'fixture_mock', message: 'Fixture replay: runtime recover acknowledged', mode: this.system.controllerMode || 'idle' };
  }
  ...
  stopTask() {
    ...
    this.pushAudit('task.stop', 'accepted', 'Fixture replay: stop-task command accepted');
    return { accepted: true, localPreviewOnly: false, commandMode: 'fixture_mock', message: 'Fixture replay: stop task acknowledged', mode: this.system.controllerMode || 'maintenance' };
  }
  ...
  saveCalibrationProfile(payload: Record<string, unknown>) {
    ...
    return { accepted: true, localPreviewOnly: false, commandMode: 'fixture_mock', message: 'Fixture replay: calibration saved locally', mode: this.system.controllerMode || 'idle' };
  }
  ...
  setControllerMode(mode: string): Record<string, unknown> {
    ...
    this.pushLog({ level: 'info', module: 'mock.fixture', event: 'controller.mode', message: `Fixture replay: controller mode command accepted: ${normalized}` });
    this.pushAudit('hardware.set_mode', 'accepted', `Fixture replay: controller mode command accepted: ${normalized}`, { mode: normalized });
    return { accepted: true, localPreviewOnly: false, commandMode: 'fixture_mock', message: `Fixture replay: controller mode command accepted: ${normalized}`, mode: normalized };
  }

  getLogs(): LogEvent[] { return clone(this.logs); }
  getAuditLogs(): CommandAuditRecord[] { return clone(this.audits); }
  getCommandReceipts(): Array<Record<string, unknown>> {
    return clone(this.audits).map((item) => ({
      id: item.id.replace('audit', 'receipt'),
      timestamp: item.finishedAt || item.createdAt,
      action: item.action,
      status: item.status,
      role: item.actorRole,
      requestId: item.requestId,
      correlationId: item.correlationId,
      commandPlane: item.action.startsWith('task.') ? 'task_control' : item.action.startsWith('hardware.') ? 'manual_control' : 'system_control',
      receiptClass: item.action.startsWith('task.') ? 'workflow_command' : item.action.startsWith('hardware.') ? 'manual_command' : 'system_command',
      executionBound: !item.action.endsWith('.event'),
