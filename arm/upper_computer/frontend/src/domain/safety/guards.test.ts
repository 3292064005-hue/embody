...
function baseSnapshot(): SafetySnapshot {
  return {
    ...
    commandPlanes: {
      manual_control: { entrypoint: 'gateway_command_service', release_track: 'official_active', producer: 'gateway_manual_control', allowed_roles: ['maintainer'], dispatch_mode: 'direct_hardware_command', receipt_class: 'manual_command', audit_required: true },
      task_control: { entrypoint: 'orchestrator_executor_contract', release_track: 'official_active', producer: 'runtime_task_control', allowed_roles: ['operator', 'maintainer'], dispatch_mode: 'workflow_authoritative', receipt_class: 'workflow_command', audit_required: true },
    }
  };
}
...
describe('safety guards', () => {
  ...
  it('blocks jog outside maintainer mode', () => {
    ...
    expect(decision.allowed).toBe(false);
    expect(decision.reason).toContain('manual_control 合同');
  });
  ...
  it('blocks gripper for operator when manual_control contract requires maintainer', () => {
    const decision = evaluateGripper({ ...baseSnapshot(), operatorRole: 'operator' });
    expect(decision.allowed).toBe(false);
    expect(decision.reason).toContain('manual_control 合同');
  });
