...
function requireCommandPlaneRole(snapshot: SafetySnapshot, commandPlane: string, fallbackReason: string): GuardDecision | null {
  const allowedRoles = snapshot.commandPlanes?.[commandPlane]?.allowed_roles || [];
  if (!allowedRoles.length) return null;
  if (!allowedRoles.includes(snapshot.operatorRole)) return block(fallbackReason);
  return null;
}
...
export function evaluateJog(snapshot: SafetySnapshot): GuardDecision {
  const hardStop = evaluateCommonHardStop(snapshot);
  if (hardStop) return hardStop;
  const roleDecision = requireCommandPlaneRole(snapshot, 'manual_control', 'manual_control 合同仅允许维护员执行点动与伺服控制');
  if (roleDecision) return roleDecision;
...
export function evaluateServoCartesian(snapshot: SafetySnapshot): GuardDecision {
  const hardStop = evaluateCommonHardStop(snapshot);
  if (hardStop) return hardStop;
  const roleDecision = requireCommandPlaneRole(snapshot, 'manual_control', 'manual_control 合同仅允许维护员执行笛卡尔微调');
  if (roleDecision) return roleDecision;
...
export function evaluateGripper(snapshot: SafetySnapshot): GuardDecision {
  const roleDecision = requireControlRole(snapshot);
  if (roleDecision) return roleDecision;
  const planeRole = requireCommandPlaneRole(snapshot, 'manual_control', 'manual_control 合同仅允许维护员执行夹爪控制');
  if (planeRole) return planeRole;
  const hardStop = evaluateCommonHardStop(snapshot);
  if (hardStop) return hardStop;
