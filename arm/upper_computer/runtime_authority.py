...
RUNTIME_AUTHORITY_SECTION_DIR = CONFIG_DIR / 'runtime_authority_sections'
RUNTIME_PRODUCT_LINES_PATH = RUNTIME_AUTHORITY_SECTION_DIR / 'product_lines.yaml'
RUNTIME_COMMAND_PLANES_PATH = RUNTIME_AUTHORITY_SECTION_DIR / 'command_planes.yaml'
RUNTIME_CAPABILITY_REGISTRY_PATH = RUNTIME_AUTHORITY_SECTION_DIR / 'capability_registry.yaml'
RUNTIME_TASK_CATALOG_CONTRACT_PATH = RUNTIME_AUTHORITY_SECTION_DIR / 'task_catalog_contract.yaml'
RUNTIME_GOVERNANCE_PROJECTION_PATH = RUNTIME_AUTHORITY_SECTION_DIR / 'runtime_governance.yaml'
...
def _validated_live_artifact_expected_status(marker: str, evidence_manifest: dict[str, Any] | None = None) -> str:
    ...
    manifest_status = str(item.get('status', '') or '').strip().lower()
    if manifest_status in {'passed', 'blocked', 'failed', 'not_executed', 'not_signed'}:
        return manifest_status
    if marker == 'target_runtime_gate_passed':
        return 'passed'
    if marker in {'hil_gate_passed', 'hil_smoke_passed'}:
        return 'passed'
    if marker == 'release_checklist_signed':
        return 'passed'
    return 'passed'


def _artifact_status_tokens(marker: str, status: str) -> tuple[str, ...]:
    normalized = str(status or 'passed').strip().lower()
    if marker == 'target_runtime_gate_passed':
        return (f'current status: `{normalized}`', f'current status: {normalized}')
    if normalized == 'not_signed':
        return ('status: **not_signed**', 'status: `not_signed`', 'status: not_signed', 'status: **not signed**', 'status: `not signed`', 'status: not signed')
    return (f'status: **{normalized}**', f'status: `{normalized}`', f'status: {normalized}')


def _unexpected_artifact_status_tokens(marker: str, expected_status: str) -> tuple[str, ...]:
    statuses = ('passed', 'blocked', 'failed', 'not_executed', 'not_signed', 'not_passed')
    unexpected: list[str] = []
    for status in statuses:
        if status == expected_status:
            continue
        unexpected.extend(_artifact_status_tokens(marker, status))
    return tuple(unexpected)


def validated_live_artifact_issues(
    marker: str,
    evidence_manifest: dict[str, Any] | None = None,
    *,
    root: Path | None = None,
) -> list[str]:
    """Validate one repository-tracked validated-live evidence artifact.

    Args:
        marker: Evidence marker such as ``target_runtime_gate_passed``.
        evidence_manifest: Optional validated-live evidence manifest mapping.
        root: Optional repository root override used by sandbox/unit tests.

    Returns:
        list[str]: Fail-closed issue list. Empty means the evidence is internally
        consistent across manifest, gate report, and checked-in artifact text.

    Boundary behavior:
        Missing artifacts, malformed reports, or stale template text all fail
        closed even when the manifest status alone says ``passed``.
    """
    payload = evidence_manifest if isinstance(evidence_manifest, dict) else {'evidence': {}}
    evidence = payload.get('evidence', {}) if isinstance(payload.get('evidence'), dict) else {}
    item = evidence.get(marker, {}) if isinstance(evidence.get(marker), dict) else {}
    ...
    manifest_status = str(item.get('status', '') or '').strip().lower()
    expected_status = _validated_live_artifact_expected_status(marker, evidence_manifest)
    if manifest_status != expected_status:
        issues.append(f'manifest_status={manifest_status or "missing"}')
    artifact = str(item.get('artifact', '') or '').strip()
    if not artifact:
        issues.append('artifact_path_missing')
        return issues
    ...
    if gate_field:
        ...
        report_path = (repo_root / report_path_value) if report_path_value else None
        report = load_release_gate_report(report_path) if report_path is not None else load_release_gate_report()
        if str(report.get(gate_field, '') or '').strip().lower() != expected_status:
            issues.append(f'gate_field_{gate_field}_not_{expected_status}')
    text = artifact_path.read_text(encoding='utf-8', errors='replace').lower()
    expected_tokens = _artifact_status_tokens(marker, expected_status)
    if not any(token in text for token in expected_tokens):
        issues.append('artifact_status_text_mismatch')
    unexpected_tokens = _unexpected_artifact_status_tokens(marker, expected_status)
    if any(token in text for token in unexpected_tokens):
        issues.append('artifact_contains_unexpected_status_token')
    return issues
...
def _validated_live_artifact_passed(marker: str, evidence_manifest: dict[str, Any] | None) -> bool:
    ...
    item = evidence.get(marker, {}) if isinstance(evidence.get(marker), dict) else {}
    manifest_status = str(item.get('status', '') or '').strip().lower()
    return manifest_status == 'passed' and not validated_live_artifact_issues(marker, evidence_manifest)
...
def _load_yaml_mapping(path: Path, *, root_key: str | None = None) -> dict[str, Any]:
    try:
        payload = yaml.safe_load(path.read_text(encoding='utf-8')) or {}
    except Exception as exc:
        raise RuntimeError(f'failed to load runtime authority projection: {path}: {exc}') from exc
    if not isinstance(payload, dict):
        raise RuntimeError(f'runtime authority projection must be a mapping: {path}')
    if root_key is None:
        return payload
    node = payload.get(root_key, {})
    if not isinstance(node, dict):
        raise RuntimeError(f'runtime authority projection missing mapping root {root_key}: {path}')
    return {str(key): value for key, value in node.items()}


def load_runtime_product_lines_projection(path: Path | None = None) -> dict[str, dict[str, Any]]:
    return {str(key): dict(value) for key, value in _load_yaml_mapping(path or RUNTIME_PRODUCT_LINES_PATH, root_key='product_lines').items() if isinstance(value, dict)}


def load_runtime_command_planes_projection(path: Path | None = None) -> dict[str, dict[str, Any]]:
    return {str(key): dict(value) for key, value in _load_yaml_mapping(path or RUNTIME_COMMAND_PLANES_PATH, root_key='command_planes').items() if isinstance(value, dict)}


def load_runtime_capability_registry_projection(path: Path | None = None) -> dict[str, Any]:
    payload = _load_yaml_mapping(path or RUNTIME_CAPABILITY_REGISTRY_PATH, root_key='capability_registry')
    return dict(payload)


def load_runtime_task_catalog_contract_projection(path: Path | None = None) -> dict[str, Any]:
    payload = _load_yaml_mapping(path or RUNTIME_TASK_CATALOG_CONTRACT_PATH, root_key='task_catalog_contract')
    return dict(payload)


def load_runtime_governance_projection(path: Path | None = None) -> dict[str, Any]:
    payload = _load_yaml_mapping(path or RUNTIME_GOVERNANCE_PROJECTION_PATH, root_key='runtime_governance')
    return dict(payload)

def derived_runtime_governance_projection(authority: dict[str, Any]) -> dict[str, Any]:
    """Return a reviewer-facing runtime-governance projection split from the monolithic authority.

    The canonical editable source remains ``runtime_authority.yaml``. This helper
    emits smaller generated projections so reviewers and tooling can inspect one
    concern area without reopening the full authority file.
    """
    governance = derived_runtime_governance(authority)
    return {
        'runtime_governance': {
            'default_runtime_lane': governance.default_runtime_lane,
            'official_product_lines': list(governance.official_product_lines),
            'experimental_product_lines': list(governance.experimental_product_lines),
            'official_runtime_lanes': list(governance.official_runtime_lanes),
            'experimental_runtime_lanes': list(governance.experimental_runtime_lanes),
            'lane_classification': dict(governance.lane_classification),
        }
    }


def derived_runtime_authority_sections(authority: dict[str, Any]) -> dict[Path, dict[str, Any]]:
    """Return split generated projections for the canonical runtime authority.

    Returns one mapping keyed by output path. Each payload contains exactly one
    concern area so downstream reviewers can diff product lines, command planes,
    capability registry, task catalog contract, and governance independently.
    """
    return {
        RUNTIME_PRODUCT_LINES_PATH: {'product_lines': derived_product_lines(authority)},
        RUNTIME_COMMAND_PLANES_PATH: {'command_planes': derived_command_planes(authority)},
        RUNTIME_CAPABILITY_REGISTRY_PATH: {'capability_registry': derived_capability_registry(authority)},
        RUNTIME_TASK_CATALOG_CONTRACT_PATH: {'task_catalog_contract': derived_task_catalog_contract(authority)},
        RUNTIME_GOVERNANCE_PROJECTION_PATH: derived_runtime_governance_projection(authority),
    }
...
def derived_command_planes(authority: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Return the authoritative command-plane contract map."""
    payload = authority.get('command_planes', {})
    if not isinstance(payload, dict):
        raise RuntimeError('runtime authority command_planes must be a mapping')
    registry = authority.get('capability_registry', {}) if isinstance(authority.get('capability_registry'), dict) else {}
    runtime_interfaces = registry.get('runtime_interfaces', {}) if isinstance(registry.get('runtime_interfaces'), dict) else {}
    result: dict[str, dict[str, Any]] = {}
    for name, item in payload.items():
        if not isinstance(item, dict):
            raise RuntimeError(f'command plane {name} must be a mapping')
        runtime_interface = str(item.get('runtime_interface', '') or '')
        dispatch_mode = str(item.get('dispatch_mode', 'observability_only') or 'observability_only')
        if not runtime_interface:
            raise RuntimeError(f'command plane {name} must declare runtime_interface')
        if runtime_interface not in runtime_interfaces:
            raise RuntimeError(f'command plane {name} references unknown runtime interface: {runtime_interface}')
        runtime_interface_state = str((runtime_interfaces.get(runtime_interface, {}) or {}).get('state', 'reserved') or 'reserved')
        if dispatch_mode != 'observability_only' and runtime_interface_state != 'active':
            raise RuntimeError(f'command plane {name} requires an active runtime interface, got {runtime_interface}={runtime_interface_state}')
        result[str(name)] = {
            'entrypoint': str(item.get('entrypoint', '') or ''),
            'release_track': str(item.get('release_track', 'official_active') or 'official_active'),
            'producer': str(item.get('producer', '') or ''),
            'allowed_roles': [str(value) for value in item.get('allowed_roles', []) if str(value).strip()],
            'dispatch_mode': dispatch_mode,
            'receipt_class': str(item.get('receipt_class', 'observability_event') or 'observability_event'),
...
def derived_capability_registry(authority: dict[str, Any]) -> dict[str, Any]:
    """Return the authoritative capability-registry projection.

    Boundary behavior:
        The projection preserves every declared registry subsection as a plain
        mapping so runtime consumers can add new capability families without
        duplicating ad-hoc parsing logic in each process.
    """
    payload = authority.get('capability_registry', {})
    if not isinstance(payload, dict):
        raise RuntimeError('runtime authority capability_registry must be a mapping')
    result: dict[str, Any] = {'schema_version': int(payload.get('schema_version', 1) or 1)}
    for section, source in payload.items():
        if section == 'schema_version':
            continue
        if not isinstance(source, dict):
            raise RuntimeError(f'capability_registry.{section} must be a mapping')
        normalized: dict[str, dict[str, Any]] = {}
        for name, item in source.items():
            if not isinstance(item, dict):
                raise RuntimeError(f'capability_registry.{section}.{name} must be a mapping')
            normalized[str(name)] = {str(key): value for key, value in item.items()}
        result[str(section)] = normalized
    return result



def derived_task_catalog_contract(authority: dict[str, Any]) -> dict[str, Any]:
    """Return task-catalog schema and compatibility policy metadata."""
    payload = authority.get('task_catalog_contract', {})
    if not isinstance(payload, dict):
        raise RuntimeError('runtime authority task_catalog_contract must be a mapping')
    policy = payload.get('compatibility_policy', {}) if isinstance(payload.get('compatibility_policy'), dict) else {}
    return {
        'schema_version': int(payload.get('schema_version', authority.get('schema_version', 1)) or authority.get('schema_version', 1)),
        'supported_schema_versions': [int(value) for value in payload.get('supported_schema_versions', [payload.get('schema_version', authority.get('schema_version', 1))])],
        'template_version': int(payload.get('template_version', 1) or 1),
        'plugin_contract_version': int(payload.get('plugin_contract_version', 1) or 1),
        'compatibility_policy': {
            'fail_on_unknown_plugin': bool(policy.get('fail_on_unknown_plugin', True)),
            'fail_on_schema_drift': bool(policy.get('fail_on_schema_drift', True)),
            'fail_on_experimental_plugin': bool(policy.get('fail_on_experimental_plugin', True)),
            'runtime_tier_order': [str(value) for value in policy.get('runtime_tier_order', ['preview', 'validated_sim', 'validated_live']) if str(value).strip()],
        },
    }



def derived_capability_descriptors(authority: dict[str, Any]) -> dict[str, dict[str, str]]:
    """Return stable capability descriptors consumed by gateway/frontend surfaces."""
    payload = authority.get('capability_descriptors', {})
    if not isinstance(payload, dict):
        raise RuntimeError('runtime authority capability_descriptors must be a mapping')
    result: dict[str, dict[str, str]] = {}
    for name, item in payload.items():
        if not isinstance(item, dict):
            raise RuntimeError(f'capability_descriptors.{name} must be a mapping')
        result[str(name)] = {
            'data_plane': str(item.get('data_plane', '') or ''),
            'control_plane': str(item.get('control_plane', '') or ''),
            'execution_binding': str(item.get('execution_binding', 'observability_only') or 'observability_only'),
            'authority_level': str(item.get('authority_level', 'unknown') or 'unknown'),
        }
    return result
...
def build_validated_live_governance_ledger(
    authority: dict[str, Any] | None = None,
    evidence_manifest: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Return one machine-readable validated-live governance ledger.

    Args:
        authority: Optional preloaded runtime-authority payload.
        evidence_manifest: Optional preloaded validated-live evidence manifest.

    Returns:
        dict[str, Any]: Aggregated backbone, promotion, release-slice, and
        evidence-artifact status used by release collectors and reviewers.

    Raises:
        RuntimeError: Propagated if the canonical release-slice declaration is
            invalid.

    Boundary behavior:
        Missing evidence files fail closed. Artifact existence is reported
        explicitly so review tooling can distinguish "declared" from "present".
    """
    payload = authority if isinstance(authority, dict) else load_runtime_authority()
    manifest = evidence_manifest if isinstance(evidence_manifest, dict) else load_validated_live_evidence()
    promotion_payload = payload.get('promotion_tiers', {}).get('validated_live', {}) if isinstance(payload.get('promotion_tiers'), dict) else {}
    promotion_status = evaluate_promotion_receipt(promotion_payload, authority=payload, evidence_manifest=manifest)
    backbone = validated_live_backbone_status(payload)
    release_slice = derived_validated_live_release_slice(payload)
    evidence_items = manifest.get('evidence', {}) if isinstance(manifest.get('evidence'), dict) else {}
    ...
    for marker in ('target_runtime_gate_passed', 'hil_gate_passed', 'release_checklist_signed'):
        item = evidence_items.get(marker, {}) if isinstance(evidence_items.get(marker), dict) else {}
        artifact = str(item.get('artifact', '') or '').strip()
        artifact_exists = bool(artifact) and (ROOT / artifact).exists()
        artifact_issues = validated_live_artifact_issues(marker, manifest)
        normalized_evidence[marker] = {
            'status': str(item.get('status', 'not_executed') or 'not_executed'),
            'artifact': artifact,
            'artifactExists': artifact_exists,
            'gateField': str(item.get('gate_field', '') or ''),
            'artifactConsistent': not artifact_issues,
            'issues': artifact_issues,
        }

    return {
        'backbone': {
            'declared': bool(backbone.declared),
            'missing': list(backbone.missing),
            'lane': backbone.lane_name,
            'planningBackendProfile': backbone.planning_backend_profile,
        },
        'promotion': {
            'mode': promotion_status.mode,
            'promoted': bool(promotion_status.promoted),
            'effective': bool(promotion_status.effective),
            'missingEvidence': list(promotion_status.missing),
        },
        'releaseSlice': dict(release_slice),
...
def derived_task_manifest(authority: dict[str, Any]) -> dict[str, Any]:
    ...
    product_lines = derived_product_lines(authority)
    contract = derived_task_catalog_contract(authority)
    registry = derived_capability_registry(authority)
    plugin_registry = registry.get('plugins', {}) if isinstance(registry.get('plugins'), dict) else {}
    tier_order = [str(value) for value in contract.get('compatibility_policy', {}).get('runtime_tier_order', ['preview', 'validated_sim', 'validated_live']) if str(value).strip()]
    ...
    for index, item in enumerate(templates):
        ...
        plugin_state = plugin_registry.get(plugin_key, {}) if isinstance(plugin_registry.get(plugin_key), dict) else {}
        if contract.get('compatibility_policy', {}).get('fail_on_unknown_plugin', True) and not plugin_state:
            raise RuntimeError(f'task template {template_id} references unknown plugin {plugin_key}')
        if contract.get('compatibility_policy', {}).get('fail_on_experimental_plugin', True) and str(plugin_state.get('state', 'active') or 'active') != 'active':
            raise RuntimeError(f'task template {template_id} references non-active plugin {plugin_key}')
        preconditions = [str(value) for value in item.get('preconditions', []) if str(value).strip()]
        tier_compatibility = [tier for tier in tier_order if required_runtime_tier in tier_order and tier_order.index(tier) >= tier_order.index(required_runtime_tier)] or [required_runtime_tier]
        normalized_templates.append({
            'id': template_id,
            'template_version': int(contract.get('template_version', 1) or 1),
            ...
            'runtime_tier_compatibility': tier_compatibility,
            ...
            'plugin_contract_version': int(contract.get('plugin_contract_version', 1) or 1),
            'plugin_state': str(plugin_state.get('state', 'active') or 'active'),
    ...
    return {
        'schema_version': int(contract.get('schema_version', authority.get('schema_version', 1)) or authority.get('schema_version', 1)),
...
def derived_promotion_receipts(authority: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for name, item in payload.items():
        ...
        derived_evidence = [marker for marker in required_evidence if marker not in status.missing]
        result[str(name)] = {
