...
INCLUDED_ARTIFACTS = {
    ...
    Path('artifacts/release_gates/frontend_validation_ledger.json'),
    Path('artifacts/release_gates/release_evidence.json'),
    Path('artifacts/release_gates/validated_live_hil_gate.json'),
    Path('artifacts/release_gates/validated_live_release_checklist_gate.json'),
    Path('artifacts/release_gates/target_runtime_gate.json'),
    Path('artifacts/release_gates/runtime_baseline_report.json'),
    Path('artifacts/target_env_report.json'),
    Path('artifacts/repository_validation/repo/verification_summary.json'),
}

INCLUDED_ARTIFACT_PREFIXES = {
    Path('artifacts/release_gates/frontend_validation_artifacts'),
    Path('artifacts/repository_validation/repo'),
}
...
def iter_release_files(root: Path = ROOT) -> Iterator[Path]:
    """Yield deterministic release files without relying on pre-existing manifests.

    Args:
        root: Package root to scan.

    Returns:
        An iterator of regular files accepted by the release filter. Directory
        traversal is sorted for reproducibility and uses `os.walk` rather than
        recursive globbing so packaging does not depend on filesystem-specific
        glob behavior.
...
def _selected_release_files_with_manifest(
    ...
    *,
    ...
    """Return the deterministic release selection including the manifest itself.

    Args:
        root: Package root used for relative sorting.
        manifest_path: Manifest path that must be represented as a self-reference.
        files: Optional base selection. When omitted, the current package selector
            is scanned.

    Returns:
        A sorted, de-duplicated file list whose relative paths match the archive
        and manifest payload. The manifest path is included even when it does not
        exist yet, enabling clean first-run generation.

    Raises:
        ValueError: If the manifest path is outside the package root.

    Boundary behavior:
        Missing non-manifest files still fail during provenance hashing or archive
        writing. The manifest itself is the only missing path admitted because it
        is created by the same packaging transaction before archiving.
    """
    ...
    try:
        ...
        raise ValueError(f'release manifest must be inside package root: {manifest_absolute}') from exc
...
def _file_provenance(path: Path, root: Path = ROOT, *, output_path: Path | None = None) -> dict[str, object]:
    """Return stable file-level provenance for one packaged file.

    Args:
        path: Absolute or root-relative file path selected for release packaging.
        root: Package root used to render the canonical relative path.
        output_path: Manifest being written, used to mark self-referential records.

    Returns:
        A JSON-serializable record containing the relative path, byte size,
        SHA-256 digest and provenance status for the exact file bytes that will
        enter the archive.
    ...
    Boundary behavior:
        The function hashes bytes only, so text encoding and platform newline
        handling cannot change the digest. Symlinks are resolved by Path I/O in
        the same way as zipfile packaging; release selection does not create
        synthetic manifest entries for directories. The manifest output itself
        is marked `self_referential_manifest` because embedding its final digest
...
def _file_provenance_records(files: Iterable[Path], root: Path = ROOT, *, output_path: Path | None = None) -> list[dict[str, object]]:
    """Build ordered provenance records for the selected release files.

    Args:
        files: Files returned by the deterministic package selector.
        root: Package root used for path normalization.
        output_path: Manifest being written, used to mark the self-reference.

    Returns:
        Records ordered exactly like the compatibility `files` list.

    Raises:
        FileNotFoundError, IsADirectoryError: Propagated from `_file_provenance`
        so missing or invalid package inputs fail closed instead of producing a
        partial manifest.
    """
...
def write_release_manifest(files: Iterable[Path], output: Path = MANIFEST_OUTPUT, root: Path = ROOT) -> Path:
    ...
    Args:
        files: Deterministically selected package files.
        output: Manifest JSON destination.
        root: Package root used for relative paths and file hashing.

    Returns:
        The manifest path written to disk.

    Raises:
        FileNotFoundError, IsADirectoryError: If any selected package input is
        missing or invalid. The manifest is not downgraded to a partial record.

    Boundary behavior:
        The legacy `files` list is preserved for old callers. `fileProvenance`
        is an ordered, one-to-one companion list. Regular files record byte size
        and SHA-256; the manifest output itself is explicitly marked as a
        self-reference instead of carrying a stale digest.
    """
    ...
    payload = {
        'schemaVersion': 2,
        'generatedBy': 'scripts/package_release.py',
...
def _validate_manifest_provenance(payload: dict, expected_files: list[Path], root: Path, manifest_path: Path) -> list[str]:
    """Validate release manifest path, size and SHA-256 provenance.
    ...
    Returns:
        A list of audit issues. Empty means the provenance matches the current
        package selection byte-for-byte.

    Boundary behavior:
        Older manifests without `fileProvenance` fail the audit instead of being
        silently accepted, while the compatibility `files` list is still checked
        independently to preserve legacy drift diagnostics.
    """
    issues: list[str] = []
    ...
    if not isinstance(provenance, list):
        return ['release manifest missing fileProvenance records']
    actual_records = [item for item in provenance if isinstance(item, dict)]
    if len(actual_records) != len(provenance):
        issues.append('release manifest fileProvenance contains non-object entries')
    normalized_actual = [
        {
    ...
    if normalized_actual != expected_records:
        issues.append('release manifest fileProvenance drift detected')
    return issues


def check_manifest(manifest_path: Path = MANIFEST_OUTPUT, root: Path = ROOT) -> list[str]:
    """Validate the release manifest against current package selection.
    ...
    Returns:
        ...
        final audit must validate a concrete evidence ledger, not a dry-run
        selection.
    """
    issues: list[str] = []
    ...
    if not manifest_path.exists():
        return [f'release manifest missing: {manifest_path.relative_to(root).as_posix()}']
    try:
        payload = json.loads(manifest_path.read_text(encoding='utf-8'))
    except Exception as exc:
        return [f'failed to parse release manifest: {exc}']
    if not isinstance(payload, dict):
        return ['release manifest must be a JSON object']
    if payload.get('schemaVersion') != 2:
        issues.append('release manifest schemaVersion must be 2')
    if payload.get('generatedBy') != 'scripts/package_release.py':
        issues.append('release manifest generatedBy drift detected')
    if payload.get('fileCount') != len(expected):
        issues.append('release manifest fileCount drift detected')
    ...
    if not isinstance(files, list) or len(actual) != len(files):
        issues.append('release manifest files must be a list of strings')
    if actual != expected:
        issues.append('release manifest drift detected')
    issues.extend(_validate_manifest_provenance(payload, expected_files, root, manifest_path))
    return issues

def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Package or validate the upper-computer deterministic release archive.')
    parser.add_argument('--check', action='store_true', help='Validate the deterministic release manifest without writing the archive.')
