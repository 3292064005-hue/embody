...
from runtime_authority import validated_live_artifact_issues
...
def _derive_repo_gate(steps: dict[str, str], *, root: Path) -> str:
    ...
    if str(payload.get('profile', '') or '').strip() != 'repo':
        return 'not_executed'
    summary_status = normalize_status(str(payload.get('overallStatus', '') or ''))
    if summary_status != 'passed':
        return summary_status if summary_status in {'failed', 'blocked', 'skipped'} else 'not_executed'
...
def _derive_evidence_gate(*, root: Path, marker: str) -> str:
    ...
    Missing files, malformed payloads, inconsistent independent gate reports,
    or stale template text all fail closed rather than passing. Release-gate
    derivation intentionally reuses the same strong artifact validation used by
    runtime-authority promotion so publishability cannot outrun promotion truth.
    """
    ...
    if raw_status in {'', 'not_executed'}:
        return 'not_executed'
    issues = validated_live_artifact_issues(marker, payload, root=root)
    if issues:
        return 'blocked'
