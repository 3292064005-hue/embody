...
def _report_path(path: Path, *, display_root: Path | None) -> str:
...
def build_report(root: Path, *, display_root: Path | None = None) -> dict[str, Any]:
    ...
    report = {
        'observabilityRoot': _report_path(root, display_root=display_root),
        'files': {
            'logs': _report_path(root / 'logs.jsonl', display_root=display_root),
            'audits': _report_path(root / 'audits.jsonl', display_root=display_root),
            'task_runs': _report_path(root / 'task_runs.jsonl', display_root=display_root),
        },
        'counts': {
            'logs': len(logs),
            'audits': len(audits),
