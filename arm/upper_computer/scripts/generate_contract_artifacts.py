...
from runtime_authority import (
    derived_capability_descriptors,
    ...
    load_runtime_capability_registry_projection,
    load_runtime_command_planes_projection,
    load_runtime_governance_projection,
    load_runtime_product_lines_projection,
    load_runtime_task_catalog_contract_projection,
)
...
def _load_task_capabilities() -> dict[str, Any]:
    ...
    product_lines = load_runtime_product_lines_projection()
    catalog_contract = load_runtime_task_catalog_contract_projection()
    capability_registry = load_runtime_capability_registry_projection()
    ...
    for item in payload.get('templates', []):
        ...
        templates.append({
            ...
            'runtimeTierCompatibility': [str(value) for value in item.get('runtime_tier_compatibility', []) if str(value).strip()],
            ...
            'pluginContractVersion': int(item.get('plugin_contract_version', catalog_contract.get('plugin_contract_version', 1)) or 1),
            'pluginState': str(item.get('plugin_state', 'active') or 'active'),
            'templateVersion': int(item.get('template_version', catalog_contract.get('template_version', 1)) or 1),
            'graphKey': str(item.get('graph_key', '') or ''),
            'taskGraph': dict(item.get('task_graph', {}) or {}),
        })
    return {
        'schemaVersion': int(payload.get('schema_version', catalog_contract.get('schema_version', 1)) or 1),
        'catalogContract': {
            'schemaVersion': int(catalog_contract.get('schema_version', 1) or 1),
            'supportedSchemaVersions': [int(value) for value in catalog_contract.get('supported_schema_versions', [1])],
            'templateVersion': int(catalog_contract.get('template_version', 1) or 1),
            'pluginContractVersion': int(catalog_contract.get('plugin_contract_version', 1) or 1),
            'compatibilityPolicy': dict(catalog_contract.get('compatibility_policy', {}) or {}),
        },
        'capabilityRegistry': dict(capability_registry),
        'productLines': {
            key: {
...
def _load_command_planes() -> dict[str, dict[str, Any]]:
    payload = load_runtime_command_planes_projection()
    return {str(name): dict(item) for name, item in payload.items() if isinstance(item, dict)}


def _load_capability_descriptors() -> dict[str, dict[str, Any]]:
    authority = load_runtime_authority(RUNTIME_AUTHORITY)
    payload = derived_capability_descriptors(authority)
    return {str(name): dict(item) for name, item in payload.items() if isinstance(item, dict)}
...
def _load_runtime_governance() -> dict[str, Any]:
    governance = load_runtime_governance_projection()
    return {
        'defaultRuntimeLane': str(governance.get('default_runtime_lane', 'preview')),
        'officialProductLines': [str(value) for value in governance.get('official_product_lines', []) if str(value).strip()],
        'experimentalProductLines': [str(value) for value in governance.get('experimental_product_lines', []) if str(value).strip()],
        'officialRuntimeLanes': [str(value) for value in governance.get('official_runtime_lanes', []) if str(value).strip()],
        'experimentalRuntimeLanes': [str(value) for value in governance.get('experimental_runtime_lanes', []) if str(value).strip()],
        'laneClassification': {str(key): str(value) for key, value in dict(governance.get('lane_classification', {}) or {}).items()},
    }
...
def build_runtime_acceptance_markdown(manifest: dict[str, Any]) -> str:
    lines = [
        ...
        '> Source of truth: `backend/embodied_arm_ws/src/arm_bringup/config/runtime_authority.yaml`',
        '',
...
def build_contract_manifest() -> dict[str, Any]:
    ...
    command_planes = _load_command_planes()
    capability_descriptors = _load_capability_descriptors()
    return {
        'schemaVersion': '1.7',
        ...
        'readiness': {
            'publicFields': [
                *[
                    field
                    for field in gateway_constants['PUBLIC_READINESS_FIELDS']
                    if field not in {'runtimeTier', 'productLine', 'runtimeFeatureState'}
                ],
                'mode',
                'controllerMode',
                'runtimePhase',
        ...
        'runtime': {
            ...
            'commandPlanes': command_planes,
            'capabilityDescriptors': capability_descriptors,
...
def build_contract_markdown(manifest: dict[str, Any]) -> str:
    ...
    lines.append('> Generated by: `scripts/generate_contract_artifacts.py`')
    lines.append('> Source of truth: backend authoritative contracts and `runtime_authority.yaml`')
    lines.append('')
    ...
    for tier_name, tier_payload in sorted(manifest['runtime']['productLineCapabilities'].items()):
        lines.append(
            ...
            f"publiclyExposed=`{tier_payload.get('publiclyExposed', True)}`, "
            f"lanes=`{', '.join(tier_payload['lanes'])}`"
        )
    lines.append('')
    lines.append('## Command planes')
    for plane_name, plane_payload in sorted(manifest['runtime']['commandPlanes'].items()):
        lines.append(
            '- '
            f'`{plane_name}`: '
            f"entrypoint=`{plane_payload['entrypoint']}`, "
            f"producer=`{plane_payload['producer']}`, "
            f"runtimeInterface=`{plane_payload.get('runtime_interface', '')}`, "
            f"receiptClass=`{plane_payload['receipt_class']}`, "
            f"dispatchMode=`{plane_payload['dispatch_mode']}`"
        )
    lines.append('')
    lines.append('## Runtime interfaces')
    runtime_interfaces = manifest['tasks']['capabilityRegistry'].get('runtime_interfaces', {}) if isinstance(manifest['tasks']['capabilityRegistry'], dict) else {}
    for interface_name, payload in sorted(runtime_interfaces.items()):
        if not isinstance(payload, dict):
            continue
        lines.append(
            '- '
    ...
    lines.append('')
    lines.append('## Capability descriptors')
    for capability_name, payload in sorted(manifest['runtime']['capabilityDescriptors'].items()):
        lines.append(
            '- '
            f'`{capability_name}`: '
            f"dataPlane=`{payload['data_plane']}`, "
            f"controlPlane=`{payload['control_plane']}`, "
            f"executionBinding=`{payload['execution_binding']}`, "
            f"authorityLevel=`{payload['authority_level']}`"
        )
    lines.append('')
    ...
    for template in manifest['tasks']['templates']:
        ...
        lines.append(
            ...
            f"templateVersion=`{template.get('templateVersion', 1)}`, "
            f"pluginState=`{template.get('pluginState', 'active')}`, "
...
def build_interface_index(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        '> Canonical narrative documentation: `docs/interfaces/ros2-interface-index.md`',
        '> This file remains machine-readable for contract checks and compatibility consumers. Do not replace it with a pointer page.',
        '> Generated from `arm_common/topic_names.py`, `service_names.py`, `action_names.py`, and `arm_bringup/launch_factory.py`.',
        '',
        'Use the canonical interface document for prose explanations and ownership guidance. Use this mirror when a script, test, or migration consumer still expects the legacy `docs/ROS2_INTERFACE_INDEX.md` path.',
        '',
    ...
    lines.extend(['', '## Runtime command planes'])
    for plane_name, payload in sorted(manifest['runtime']['commandPlanes'].items()):
        lines.append(f"- `{plane_name}`: entrypoint=`{payload['entrypoint']}`, dispatchMode=`{payload['dispatch_mode']}`")
...
def build_gateway_runtime_contract(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"COMMAND_PLANES = {manifest['runtime']['commandPlanes']!r}",
        f"CAPABILITY_DESCRIPTORS = {manifest['runtime']['capabilityDescriptors']!r}",
        f"TASK_CATALOG_CONTRACT = {manifest['tasks']['catalogContract']!r}",
        f"TASK_CAPABILITY_REGISTRY = {manifest['tasks']['capabilityRegistry']!r}",
...
def build_frontend_runtime_contract(manifest: dict[str, Any]) -> str:
    ...
    return '\n'.join([
        ...
        f"export const COMMAND_PLANES = {json.dumps(manifest['runtime']['commandPlanes'], ensure_ascii=False, indent=2)} as const;" ,
        f"export const CAPABILITY_DESCRIPTORS = {json.dumps(manifest['runtime']['capabilityDescriptors'], ensure_ascii=False, indent=2)} as const;" ,
        f"export const TASK_CATALOG_CONTRACT = {json.dumps(manifest['tasks']['catalogContract'], ensure_ascii=False, indent=2)} as const;" ,
        f"export const TASK_CAPABILITY_REGISTRY = {json.dumps(manifest['tasks']['capabilityRegistry'], ensure_ascii=False, indent=2)} as const;" ,
...
def build_contract_schema(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        'runtime': {
            ...
            'commandPlaneKeys': sorted(manifest['runtime']['commandPlanes'].keys()),
            'capabilityDescriptorKeys': sorted(manifest['runtime']['capabilityDescriptors'].keys()),
            'validatedLiveReleaseSlice': dict(manifest['runtime']['validatedLiveReleaseSlice']),
        },
        'tasks': {
            'schemaVersion': manifest['tasks']['schemaVersion'],
...
def build_contract_index() -> str:
    return '\n'.join([
        ...
        '> Canonical API contract: `docs/interfaces/api-contract.md`',
        '> This file remains generated because audit and compatibility tooling still consume the legacy `docs/CONTRACT_INDEX.md` path.',
        '',
        ...
        '- `INDEX.md`',
        '- `interfaces/api-contract.md`',
        '- `interfaces/ros2-interface-index.md`',
        '',
        '## Source of truth',
