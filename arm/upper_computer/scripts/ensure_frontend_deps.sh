#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FRONTEND_DIR="${ROOT_DIR}/frontend"
LOCKFILE="${FRONTEND_DIR}/package-lock.json"

export NPM_CONFIG_USERCONFIG="${FRONTEND_DIR}/.npmrc"
CANONICAL_REGISTRY="https://registry.npmjs.org/"
SELECTED_REGISTRY="${FRONTEND_NPM_REGISTRY:-${CANONICAL_REGISTRY}}"
unset NODE_AUTH_TOKEN NPM_TOKEN NPM_CONFIG__AUTH NPM_CONFIG__AUTH_TOKEN npm_config__auth npm_config__auth_token NPM_CONFIG_REGISTRY npm_config_registry || true
export NPM_CONFIG_REGISTRY="${SELECTED_REGISTRY}"
export npm_config_registry="${SELECTED_REGISTRY}"
export NPM_CONFIG_GLOBALCONFIG=/dev/null
cd "${FRONTEND_DIR}"

audit_lockfile_registry() {
  python - "${LOCKFILE}" <<'PY2'
from pathlib import Path
import sys
lockfile = Path(sys.argv[1])
text = lockfile.read_text(encoding='utf-8')
forbidden_tokens = [
    'packages.applied-caas-gateway1.internal.api.openai.org',
    'artifactory/api/npm',
    'reader:',
    '_authToken',
]
violations = [token for token in forbidden_tokens if token in text]
if violations:
    raise SystemExit(f'package-lock.json contains forbidden registry material: {", ".join(violations)}')
PY2
}

probe_registry_dns() {
  python - <<'PY2'
import os
import socket
import sys
from urllib.parse import urlparse
registry = os.environ.get('NPM_CONFIG_REGISTRY') or os.environ.get('npm_config_registry') or 'https://registry.npmjs.org/'
host = urlparse(registry).hostname or 'registry.npmjs.org'
try:
    socket.getaddrinfo(host, 443)
except OSError as exc:
    sys.stderr.write(f'[frontend] dependency bootstrap blocked: DNS unavailable for {host}: {exc}\n')
    raise SystemExit(3)
PY2
}

ensure_bin_link() {
  local name="$1"
  local target="$2"
  if [[ ! -f "${target}" ]]; then
    return 0
  fi
  mkdir -p node_modules/.bin
  ln -sf "../${target#node_modules/}" "node_modules/.bin/${name}"
  chmod +x "node_modules/.bin/${name}" || true
}

ensure_tool_bins() {
  ensure_bin_link vue-tsc node_modules/vue-tsc/bin/vue-tsc.js
  ensure_bin_link vite node_modules/vite/bin/vite.js
  ensure_bin_link vitest node_modules/vitest/vitest.mjs
  ensure_bin_link playwright node_modules/playwright/cli.js
}

needs_install=0

if [[ ! -f package-lock.json ]]; then
  echo "frontend dependency bootstrap requires package-lock.json" >&2
  exit 1
fi

audit_lockfile_registry
probe_registry_dns

if [[ ! -d node_modules ]]; then
  needs_install=1
elif [[ ! -f node_modules/vue-tsc/bin/vue-tsc.js ]]; then
  needs_install=1
elif [[ ! -f node_modules/vite/bin/vite.js ]]; then
  needs_install=1
else
  ensure_tool_bins
  if [[ ! -e node_modules/.bin/vue-tsc ]]; then
    needs_install=1
  elif ! node node_modules/vue-tsc/bin/vue-tsc.js --version >/dev/null 2>&1; then
    needs_install=1
  elif ! node node_modules/vite/bin/vite.js --version >/dev/null 2>&1; then
    needs_install=1
  fi
fi

if [[ "${needs_install}" -eq 1 ]]; then
  echo "[frontend] installing dependencies with npm ci (registry: ${SELECTED_REGISTRY})"
  npm ci --userconfig "${NPM_CONFIG_USERCONFIG}" --registry "${NPM_CONFIG_REGISTRY}"
  ensure_tool_bins
fi
