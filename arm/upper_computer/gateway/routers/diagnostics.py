...
@router.get('/api/logs/receipts')
async def get_command_receipts(request: Request):
    ctx = context_from_request(request)
    return wrap_response(ctx.state.get_command_receipts(), request_id_from_request(request))
