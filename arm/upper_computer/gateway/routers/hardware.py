...
async def post_set_mode(body: SetModeRequest, request: Request):
    ...
    result = await command.execute(
        CommandExecutionPlan(
            ...
            success_message=lambda result: 'mode projected locally (preview only)' if result.get('localPreviewOnly') else 'mode change accepted; wait for authoritative system state',
            runtime_topics=('system',),
            command_plane='system_control',
            state_mutator=mutate_mode,
        ),
        lambda: ctx.ros.dispatch_runtime_command(command_plane='system_control', action='hardware.set_mode', payload=body.model_dump()),
    )
...
async def post_servo_cartesian(body: ServoCartesianRequest, request: Request):
    ...
    Returns:
        A wrapped command-acceptance response bound to the current request id.
