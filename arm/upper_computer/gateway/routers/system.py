from __future__ import annotations

from fastapi import APIRouter, Request

from ..api_common import request_id_from_request
from ..command_service import CommandExecutionPlan, GatewayCommandService
from ..errors import ErrorCode, FailureClass
from ..lifespan import context_from_request
from ..models import coerce_system_state_aliases, wrap_response
from ..security import validate_command_policy

router = APIRouter()


def _enforce_system_command_policy(command: GatewayCommandService, *, action: str, readiness: dict, policy_name: str) -> None:
    """Apply authoritative readiness gating to one system command before transport dispatch."""
    command.require_policy(
        action=action,
        policy=validate_command_policy(readiness, policy_name),
        status_code=409,
        error=ErrorCode.READINESS_BLOCKED,
        failure_class=FailureClass.READINESS_BLOCKED,
    )


@router.get('/api/system/summary')
async def get_system_summary(request: Request):
    ctx = context_from_request(request)
    return wrap_response(ctx.state.get_system(), request_id_from_request(request))


@router.get('/api/system/readiness')
async def get_system_readiness(request: Request):
    ctx = context_from_request(request)
    return wrap_response(ctx.state.get_public_readiness(), request_id_from_request(request))


@router.post('/api/system/home')
async def post_system_home(request: Request):
    ctx = context_from_request(request)
    request_id = request_id_from_request(request)
    command = GatewayCommandService(request, ctx=ctx)
    _enforce_system_command_policy(command, action='system.home', readiness=ctx.state.get_public_readiness(), policy_name='home')
    result = await command.execute(
        CommandExecutionPlan(
            action='system.home',
            log_module='gateway.system',
            runtime_topics=('system', 'hardware'),
            command_plane='system_control',
        ),
        lambda: ctx.ros.dispatch_runtime_command(command_plane='system_control', action='system.home', payload=None),
    )
    return wrap_response(result, request_id)


@router.post('/api/system/reset-fault')
async def post_reset_fault(request: Request):
    ctx = context_from_request(request)
    request_id = request_id_from_request(request)
    command = GatewayCommandService(request, ctx=ctx)
    _enforce_system_command_policy(command, action='system.reset_fault', readiness=ctx.state.get_public_readiness(), policy_name='resetFault')
    result = await command.execute(
        CommandExecutionPlan(
            action='system.reset_fault',
            log_module='gateway.system',
            runtime_topics=('system', 'hardware'),
            command_plane='system_control',
        ),
        lambda: ctx.ros.dispatch_runtime_command(command_plane='system_control', action='system.reset_fault', payload=None),
    )
    return wrap_response(result, request_id)


@router.post('/api/system/recover')
async def post_recover(request: Request):
    ctx = context_from_request(request)
    request_id = request_id_from_request(request)
    command = GatewayCommandService(request, ctx=ctx)
    _enforce_system_command_policy(command, action='system.recover', readiness=ctx.state.get_public_readiness(), policy_name='recover')
    result = await command.execute(
        CommandExecutionPlan(
            action='system.recover',
            log_module='gateway.system',
            runtime_topics=('system', 'hardware'),
            command_plane='system_control',
        ),
        lambda: ctx.ros.dispatch_runtime_command(command_plane='system_control', action='system.recover', payload=None),
    )
    return wrap_response(result, request_id)


@router.post('/api/system/emergency-stop')
async def post_estop(request: Request):
    ctx = context_from_request(request)
    request_id = request_id_from_request(request)
    command = GatewayCommandService(request, ctx=ctx)

    def mutate_estop_state(effective_ctx, result):
        system = effective_ctx.state.get_system()
        system['runtimePhase'] = 'safe_stop'
        system['controllerMode'] = 'maintenance'
        system['taskStage'] = 'failed'
        system['emergencyStop'] = True
        system['faultCode'] = 'ESTOP'
        system['faultMessage'] = result['message']
        effective_ctx.state.set_system(coerce_system_state_aliases(system))
        return None

    result = await command.execute(
        CommandExecutionPlan(
            action='system.emergency_stop',
            log_level='fault',
            log_module='gateway.system',
            runtime_topics=('system', 'hardware'),
            command_plane='system_control',
            state_mutator=mutate_estop_state,
        ),
        lambda: ctx.ros.dispatch_runtime_command(command_plane='system_control', action='system.emergency_stop', payload=None),
    )
    return wrap_response(result, request_id)

