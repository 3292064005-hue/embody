...
def validate_start_task(readiness: dict[str, Any], operator_role: str) -> PolicyResult:
    """Validate task start against role, freshness, backend policy, and task-execution truth."""
    ...
    if task_execution_state:
        if bool(task_execution_state.get('startAllowed', False)):
            return PolicyResult(True, 'ok')
        if not bool(task_execution_state.get('workbenchVisible', False)):
            return PolicyResult(False, str(task_execution_state.get('reason', 'task execution surface is not publicly exposed') or 'task execution surface is not publicly exposed'))
        if not bool(task_execution_state.get('interactive', False)):
            return PolicyResult(False, str(task_execution_state.get('reason', 'task execution remains blocked') or 'task execution remains blocked'))
        return PolicyResult(False, str(task_execution_state.get('reason', 'startTask is not allowed') or 'startTask is not allowed'))
