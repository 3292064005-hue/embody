...
def test_gateway_state_attach_request_context_does_not_reference_undefined_metadata() -> None:
    state = GatewayState()
    request_id, correlation_id, task_run_id = state.attach_request_context('task-attach', 'req-attach')
    assert request_id == 'req-attach'
    stored_request_id, stored_correlation_id, stored_task_run_id = state.request_context('task-attach')
    assert stored_request_id == request_id
    assert stored_correlation_id == correlation_id
    assert stored_task_run_id == task_run_id
