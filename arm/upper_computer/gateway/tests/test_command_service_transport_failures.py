...
def test_command_timeout_maps_to_structured_504_and_failure_audit(monkeypatch):
    async def _timeout_dispatch(*, command_plane: str, action: str, payload=None):
        assert command_plane == 'system_control'
        assert action == 'system.home'
        raise asyncio.TimeoutError()

    monkeypatch.setattr(CTX.ros, 'dispatch_runtime_command', _timeout_dispatch)
...
def test_ros_bridge_unavailable_maps_to_structured_503(monkeypatch):
    async def _unavailable_dispatch(*, command_plane: str, action: str, payload=None):
        assert command_plane == 'system_control'
        assert action == 'system.home'
        raise RosBridgeError('ROS2 service unavailable: /arm/home')

    monkeypatch.setattr(CTX.ros, 'dispatch_runtime_command', _unavailable_dispatch)
    with TestClient(app) as client:
        response = client.post('/api/system/home', headers={'X-Operator-Role': 'operator'})
