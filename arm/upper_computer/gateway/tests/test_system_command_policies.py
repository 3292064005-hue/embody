...
def test_system_commands_fail_closed_against_authoritative_command_policies(monkeypatch):
    ...
    async def _unexpected_call(*, command_plane: str, action: str, payload=None):
        raise AssertionError(f'transport should not be called when readiness policy blocks the command: {command_plane}:{action}')

    monkeypatch.setattr(CTX.ros, 'dispatch_runtime_command', _unexpected_call)
