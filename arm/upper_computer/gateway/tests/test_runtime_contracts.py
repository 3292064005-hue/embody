...
def test_start_task_allows_validated_live_when_public_surface_and_receipt_are_effective(monkeypatch):
    ...
    async def _fake_start_task(*, command_plane: str, action: str, payload=None):
        assert command_plane == 'task_control'
        assert action == 'task.start'
        assert payload['task_type'] == 'PICK_AND_PLACE'
        assert payload['target_selector'] == 'red'
    ...
    monkeypatch.setattr(CTX.ros, 'dispatch_runtime_command', _fake_start_task)
    try:
        with TestClient(app) as client:
            CTX.state.set_readiness_snapshot(updated, authoritative=True)
        ...
        assert payload['taskId'] == 'gw-live-1'
        current = CTX.state.get_current_task()
        assert current is not None
        assert current['taskId'] == 'gw-live-1'
    finally:
...
def test_start_task_stale_authoritative_snapshot_is_readiness_blocked(monkeypatch):
    ...
    async def _unexpected_start_task(*, command_plane: str, action: str, payload=None):
        raise AssertionError('transport should not be called when authoritative readiness is stale')

    monkeypatch.setattr(CTX.ros, 'dispatch_runtime_command', _unexpected_start_task)
    try:
        with TestClient(app) as client:
...
def test_public_readiness_surface_prefers_runtime_surface_state():
    with TestClient(app) as client:
        readiness = client.get('/health/ready').json()['data']
    assert 'runtimeSurfaceState' in readiness
    assert 'runtimeFeatureState' not in readiness
    assert 'runtimeTier' not in readiness
    assert 'productLine' not in readiness
    surface = readiness['runtimeSurfaceState']
    assert 'runtimeTier' in surface
    assert 'runtimeDeliveryTrack' in surface
    assert 'executionBackbone' in surface
