...
def test_ws_contract_doc_matches_bootstrap_versioning():
    text = Path('docs/interfaces/api-contract.md').read_text(encoding='utf-8')
...
def test_target_env_bootstrap_is_documented():
    ...
    assert 'Canonical location: `docs/operations/verification-and-release.md`' in validation
    assert 'make target-env-bootstrap' not in validation
...
def test_frontend_dependency_bootstrap_uses_local_npmrc():
    ...
    assert 'CANONICAL_REGISTRY="https://registry.npmjs.org/"' in text
    assert 'unset NODE_AUTH_TOKEN NPM_TOKEN' in text
    assert 'unset NODE_AUTH_TOKEN NPM_TOKEN NPM_CONFIG__AUTH NPM_CONFIG__AUTH_TOKEN npm_config__auth npm_config__auth_token NPM_CONFIG_REGISTRY npm_config_registry' in text
...
def test_docker_target_validation_lane_is_present_and_documented():
    ...
    assert 'docs/operations/verification-and-release.md' in validation
    assert 'make ros-target-validate-docker' not in validation
...
def test_observability_sink_is_documented_and_excluded_from_release():
    ...
    assert 'INCLUDED_ARTIFACTS' in packaging
    assert 'frontend_validation_ledger.json' in packaging
    assert 'runtime_baseline_report.json' in packaging
    assert 'target_env_report.json' in packaging
    assert 'artifacts/repository_validation/repo' in packaging


def test_package_support_matrix_promotes_esp32_gateway():
    overview = Path('docs/architecture/system-overview.md').read_text(encoding='utf-8')
    assert '`arm_esp32_gateway` | runtime-core | yes' in overview
    assert 'Runtime Core representative packages' in overview and 'arm_esp32_gateway' in overview
    assert 'Experimental representative packages' in overview and '`arm_hmi`' in overview
...
def test_target_env_report_in_delivery_tree_uses_relative_workspace_path():
    payload = json.loads(Path('artifacts/target_env_report.json').read_text(encoding='utf-8'))
    workspace_dir = str((payload.get('facts', {}) if isinstance(payload.get('facts'), dict) else {}).get('workspaceDir', '') or '')
    assert workspace_dir == 'backend/embodied_arm_ws'


def test_runtime_baseline_report_in_delivery_tree_uses_relative_fixture_paths():
    payload = json.loads(Path('artifacts/release_gates/runtime_baseline_report.json').read_text(encoding='utf-8'))
    report = payload.get('report', {}) if isinstance(payload.get('report'), dict) else {}
    assert report.get('observabilityRoot') == 'gateway/tests/fixtures/observability_sample'
    files = report.get('files', {}) if isinstance(report.get('files'), dict) else {}
    assert files == {
        'logs': 'gateway/tests/fixtures/observability_sample/logs.jsonl',
        'audits': 'gateway/tests/fixtures/observability_sample/audits.jsonl',
        'task_runs': 'gateway/tests/fixtures/observability_sample/task_runs.jsonl',
    }
