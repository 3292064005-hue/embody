...
from .generated.runtime_contract import PRODUCT_LINE_CAPABILITIES, TASK_CAPABILITY_REGISTRY, TASK_CAPABILITY_TEMPLATES, TASK_CATALOG_CONTRACT
...
def _fallback_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        'tasks': {
            'schemaVersion': int(TASK_CATALOG_CONTRACT.get('schemaVersion', 2) or 2),
            'catalogContract': dict(TASK_CATALOG_CONTRACT),
            'capabilityRegistry': dict(TASK_CAPABILITY_REGISTRY),
            'productLines': {
                str(key): dict(value) for key, value in PRODUCT_LINE_CAPABILITIES.items()
            },
...
def public_task_templates() -> list[dict[str, Any]]:
    ...
    for item in _templates_by_id().values():
        templates.append({
            ...
            'templateVersion': int(item.get('templateVersion', TASK_CATALOG_CONTRACT.get('templateVersion', 1)) or 1),
            'pluginContractVersion': int(item.get('pluginContractVersion', TASK_CATALOG_CONTRACT.get('pluginContractVersion', 1)) or 1),
            'pluginState': str(item.get('pluginState', 'active') or 'active'),
            'runtimeTierCompatibility': list(item.get('runtimeTierCompatibility', []) or []),
...
def task_capability_summary() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        return {'schemaVersion': int(TASK_CATALOG_CONTRACT.get('schemaVersion', 2) or 2), 'catalogContract': dict(TASK_CATALOG_CONTRACT), 'capabilityRegistry': dict(TASK_CAPABILITY_REGISTRY), 'templates': [], 'productLines': {}}
    return {
        'schemaVersion': int(payload.get('schemaVersion', TASK_CATALOG_CONTRACT.get('schemaVersion', 2)) or 2),
        'catalogContract': dict(payload.get('catalogContract', {}) or TASK_CATALOG_CONTRACT),
        'capabilityRegistry': dict(payload.get('capabilityRegistry', {}) or TASK_CAPABILITY_REGISTRY),
...
def resolve_task_request(*, template_id: str | None, task_type: str | None, target_category: str | None) -> ResolvedTaskRequest:
    ...
    manifest = _manifest()
    catalog_contract = manifest.get('tasks', {}).get('catalogContract', {}) if isinstance(manifest.get('tasks', {}), dict) else {}
    supported_schema_versions = [int(value) for value in catalog_contract.get('supportedSchemaVersions', [int(TASK_CATALOG_CONTRACT.get('schemaVersion', 2) or 2)])]
    schema_version = int(manifest.get('tasks', {}).get('schemaVersion', TASK_CATALOG_CONTRACT.get('schemaVersion', 2)) or 2)
    if schema_version not in supported_schema_versions:
        raise ValueError(f'unsupported task catalog schemaVersion: {schema_version}')

    plugin_key = str(resolved_template.get('pluginKey', resolved_template.get('sequenceMode', 'single_target')) or 'single_target')
    registry = manifest.get('tasks', {}).get('capabilityRegistry', {}) if isinstance(manifest.get('tasks', {}), dict) else {}
    plugins = registry.get('plugins', {}) if isinstance(registry, dict) else {}
    plugin_contract = plugins.get(plugin_key, {}) if isinstance(plugins, dict) else {}
    plugin_state = str(resolved_template.get('pluginState', plugin_contract.get('state', 'active')) or 'active')
    if plugin_state != 'active' and bool(catalog_contract.get('compatibilityPolicy', {}).get('failOnExperimentalPlugin', True)):
        raise ValueError(f'task capability plugin {plugin_key} is not active: {plugin_state}')
