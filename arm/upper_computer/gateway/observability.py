...
class StructuredEventSink:
    ...
    def __init__(self, root: Path, *, strict_sync: bool = False, queue_size: int = 2048) -> None:
        ...
        self._streams = {
            ...
            'task_runs': self.root / 'task_runs.jsonl',
        }
        self._policies = {
            ...
            'task_runs': StreamPolicy(flush_interval_sec=0.10, fsync_interval_sec=0.50, batch_size=32),
        }
        ...
        self._stream_errors: dict[str, str | None] = {name: None for name in self._streams}
        self._closed = False
    ...
    def close(self) -> None:
        ...
        for handle in self._handles.values():
            try:
                ...
                self._record_error(f'close failed: {exc}')
        self._handles.clear()

    def metrics(self) -> dict[str, Any]:
        with self._metrics_lock:
            active_stream_errors = {
                stream: error
                for stream, error in self._stream_errors.items()
                if error
            }
            stream_error_items = sorted(active_stream_errors.items())
            if self._global_error:
                error_messages = [self._global_error]
                error_messages.extend(
                    f'{stream}: {error}'
                    for stream, error in stream_error_items
                )
                last_error = '; '.join(error_messages)
            elif len(stream_error_items) == 1:
                _, last_error = stream_error_items[0]
            elif stream_error_items:
                last_error = '; '.join(
                    f'{stream}: {error}'
                    for stream, error in stream_error_items
                )
            ...
            return {
                ...
                'streamErrors': dict(active_stream_errors),
                'sinkWritable': not bool(last_error),
                'degraded': bool(last_error or self._dropped_records),
            }

    def _record_error(self, error: str, *, stream: str | None = None) -> None:
        with self._metrics_lock:
            if stream is None:
                self._global_error = error
            else:
                self._stream_errors[stream] = error

    def _clear_error(self, *, stream: str | None = None) -> None:
        with self._metrics_lock:
            if stream is None:
                self._global_error = None
            else:
                self._stream_errors[stream] = None
    ...
    def _write_batch(self, stream: str, records: list[dict[str, Any]], *, force_flush: bool, force_fsync: bool) -> None:
        ...
        try:
            ...
            self._clear_error(stream=stream)
        except Exception as exc:
            self._record_error(str(exc), stream=stream)
            raise

    def _flush_handles(self, *, force_fsync: bool) -> bool:
        had_error = False
    ...
    def _writer_loop(self) -> None:
        pending: dict[str, list[dict[str, Any]]] = {name: [] for name in self._streams}
            ...
            for stream, records in pending.items():
                ...
                if due_flush:
                    ...
                    try:
                        ...
                        writer_failed = True
                        writer_errors.append(f'{stream}: {exc}')
                        self._record_error(f'writer loop flush failed: {exc}', stream=stream)
                    last_flush[stream] = time.monotonic()
                    if force_fsync:
                        last_fsync[stream] = last_flush[stream]
            if flush_request is not None:
                flush_ok = self._flush_handles(force_fsync=True)
                flush_request.success = bool(flush_ok and not writer_failed)
                flush_request.error = None if flush_request.success else ('; '.join(writer_errors) or self.metrics().get('lastError') or 'observability flush failed')
        ...
        for stream, records in pending.items():
            if records:
                try:
                    ...
                    self._record_error(f'writer shutdown flush failed: {exc}')
