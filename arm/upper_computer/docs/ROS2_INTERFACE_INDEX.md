...
## Runtime command planes
- `manual_control`: entrypoint=`runtime_command_gateway`, dispatchMode=`direct_hardware_command`
- `system_control`: entrypoint=`runtime_command_gateway`, dispatchMode=`runtime_control`
- `task_control`: entrypoint=`runtime_command_gateway`, dispatchMode=`workflow_authoritative`
- `vision_observability`: entrypoint=`runtime_ingress_projection`, dispatchMode=`observability_only`
- `voice_observability`: entrypoint=`esp32_gateway_observability`, dispatchMode=`observability_only`
